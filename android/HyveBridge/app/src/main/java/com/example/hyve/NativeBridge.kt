...
/**
 * JavaScript interface exposed to the WebView as `HyveNative`.
 *
...
class NativeBridge(
    ...
    fun saveServerConfig(jsonStr: String) {
        try {
            ...
            if (ext.isBlank() && local.isBlank() && prefs.hasServerUrl) {
                android.util.Log.w("Hyve", "NativeBridge.saveServerConfig: ignoring empty URLs (existing config preserved)")
                return
            }
            ...
            android.util.Log.d("Hyve", "NativeBridge.saveServerConfig: saved (ext=${ext.take(30)}, local=${local.take(30)})")
    ...
    fun saveAuthToken(token: String) {
        try {
            android.util.Log.d("Hyve", "NativeBridge.saveAuthToken called, token=${token.take(10)}...")
    ...
    fun clearAuthToken() {
        try {
            android.util.Log.d("Hyve", "NativeBridge.clearAuthToken called")
    ...
    companion object {
         ...
         * (`window.__HYVE_NATIVE_APP`, `window.__HYVE_NATIVE_CONFIG`, etc.)
        ...
        fun buildBridgeScript(
            ...
            return """
                (function() {
                    window.__HYVE_NATIVE_APP = true;

                    window.__HYVE_NATIVE_CONFIG = {
