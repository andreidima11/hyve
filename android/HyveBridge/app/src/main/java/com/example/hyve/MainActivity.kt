...
/**
 * Main activity — hosts a full-screen WebView that loads the Hyve UI.
 *
 ...
 * through `window.__HYVE_*` globals.
 */
class MainActivity : AppCompatActivity() {
    ...
    private val notificationPermission = registerForActivityResult(
        ...
        if (granted) {
            android.util.Log.d("Hyve", "POST_NOTIFICATIONS permission granted")
        } else {
            android.util.Log.w("Hyve", "POST_NOTIFICATIONS permission denied — notifications won't show")
        }
    }
    ...
    private val bridgePermissionLauncher = registerForActivityResult(
        ...
        android.util.Log.d("Hyve", "Bridge permission result: $name = $granted")
        webView.post {
    ...
    private fun requestAndroidPermission(name: String) {
        ...
        if (perm == null) {
            android.util.Log.w("Hyve", "Unknown permission name: $name")
            return
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //
        // IMPORTANT:
        // - Top/left/right system-bar padding should stay, otherwise the app header
        //   can end up under the status bar / display cutouts.
        // - Bottom padding should include the system gesture/nav area when the
        //   keyboard is closed, otherwise the composer sits under the Android
        //   gesture bar on some devices.
        // - While the IME is open, use the keyboard inset instead.
        ViewCompat.setOnApplyWindowInsetsListener(contentRoot) { v, insets ->
            val bars = insets.getInsets(WindowInsetsCompat.Type.systemBars())
            val ime  = insets.getInsets(WindowInsetsCompat.Type.ime())
            val isKeyboardOpen = ime.bottom > bars.bottom
            val bottomPad = if (isKeyboardOpen) ime.bottom else bars.bottom
    ...
    private fun setupWebView() {
        val bridge = NativeBridge(
            ...
            onAuthTokenReceived = { token ->
                android.util.Log.d("Hyve", "Auth token received from JS bridge, starting NotificationService")
                PushRegistrationHelper.registerCurrentToken(this, token)
                startNotificationService()
            },
            onAuthTokenCleared = { oldToken ->
                android.util.Log.d("Hyve", "Auth token cleared from JS bridge, stopping NotificationService")
            ...
            onSetWsServiceEnabled = { enabled ->
                ...
                android.util.Log.d("Hyve", "Native WS service enabled set to $enabled")
                if (enabled) {
                    startNotificationService()
                } else {
                    NotificationService.stop(this)
        ...
        webView.addJavascriptInterface(bridge, "HyveNative")
        ...
        webView.settings.apply {
            ...
            userAgentString = "$userAgentString Hyve/1.0"
        }
        ...
        webView.webViewClient = object : WebViewClient() {
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                if (pendingMsg != null) {
                    ...
                    webView.postDelayed({
                        injectNotificationMessage(pendingTitle ?: "Hyve", pendingMsg, pendingSessionId)
                    }, 1500)
                }
            }
    ...
    private fun launchCameraCapture(filePathCallback: android.webkit.ValueCallback<Array<Uri>>) {
        ...
        try {
            ...
            val photoFile = File.createTempFile("HYVE_${timeStamp}_", ".jpg", storageDir)
    ...
    private fun extractAuthTokenFromWebPage() {
        ...
        val script = """
            (function() {
                try {
                    var token = localStorage.getItem('hyve_token');
        ...
        webView.evaluateJavascript(script) { result ->
            android.util.Log.d("Hyve", "Token extraction result: $result")
        }
        ...
        webView.postDelayed({
            ...
            webView.evaluateJavascript(script) { result ->
                android.util.Log.d("Hyve", "Token extraction retry result: $result")
            }
    ...
    private fun startNotificationService() {
        ...
        if (!prefs.websocketServiceEnabled) {
            android.util.Log.d("Hyve", "NotificationService disabled by native preference")
            NotificationService.stop(this)
            return
        }
        if (token.isNotBlank()) {
            android.util.Log.d("Hyve", "Starting NotificationService (token=${token.take(10)}...)")
            PushRegistrationHelper.registerCurrentToken(this, token)
            NotificationService.start(this)
        } else {
            android.util.Log.d("Hyve", "No auth token yet — NotificationService deferred")
        }
    }
    ...
    private fun handleDeepLink(intent: Intent?) {
        ...
        if (uri.scheme == "hyve" && uri.host == "settings") {
            openSettings()
        }
    }
    ...
    private fun handleNotificationIntent(intent: Intent?) {
        ...
        val title = intent.getStringExtra("notification_title") ?: "Hyve"
        val sessionId = intent.getStringExtra("notification_session_id")

        // Clear the flag so we don't re-process on config change
        intent.removeExtra("from_notification")

        android.util.Log.d("Hyve", "Notification tap: title=$title, message=${message.take(80)}, session=$sessionId")
        ...
        webView.post {
            webView.evaluateJavascript(
                """(function() {
                    ...
                    window.__pendingHyveNotification = {
                        title: '$escapedTitle',
                    ...
                    var attempts = 0;
                    function tryDeliver() {
                        if (typeof window.__hyveShowNotification === 'function') {
                            window.__hyveShowNotification('$escapedTitle', '$escapedMessage', $sessionArg);
