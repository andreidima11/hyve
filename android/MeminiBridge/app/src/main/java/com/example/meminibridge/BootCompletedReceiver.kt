    ...
    override fun onReceive(context: Context, intent: Intent?) {
        ...
        try {
            ...
            if (hasToken && hasServerUrl && prefs.websocketServiceEnabled) {
                ...
                Log.d("MeminiBoot", "Started NotificationService on action=$action")
            } else {
                Log.d("MeminiBoot", "Skip service start on action=$action (token/url missing or ws disabled)")
            }
        } catch (e: Exception) {
            Log.e("MeminiBoot", "Receiver error: ${e.message}")
        }
    }
}
