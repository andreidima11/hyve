"""Mosquitto extract — MQTT discovery and Z2M entity extraction."""
...
def extract_mosquitto_candidates(payload: Any) -> list[dict[str, Any]]:
    ...
    for topic, msg in discovery.items():
        ...
        prop = mqtt_parse._extract_z2m_property_from_template(vt)
        dom = str(entity.get("domain") or "").lower()
        if dev_id and prop and prop in expose_covered.get(dev_id, set()):
            continue
        if (
            z2m_devices
            and mqtt_parse._device_known_in_z2m(z2m_devices, dev_id)
...
def _entity_from_discovery(
    ...
    live_payload = mqtt_parse._live_z2m_payload(
        states, device_id, device_meta, state_topic, extra_names=[device_name, feature_name],
    )
    z2m_prop = mqtt_parse._extract_z2m_property_from_template(template)
    raw_state = live_payload if live_payload else mqtt_parse._decode_z2m_payload(states.get(state_topic) if state_topic else None)
    if z2m_prop and isinstance(live_payload, dict) and z2m_prop in live_payload:
        state_value = live_payload.get(z2m_prop)
    else:
        state_value = mqtt_parse._apply_value_template(template, raw_state)
    ...
    norm_state = mqtt_parse._normalize_state(state_value, domain, capabilities)
...
def _merge_payload(stored: Any, live: Any) -> dict[str, Any]:
    ...
    merged_states = {
        ...
        if not mqtt_parse._is_z2m_non_state_topic(k)
    }
    for topic, value in live_states.items():
        if mqtt_parse._is_z2m_non_state_topic(topic):
            continue
...
def _resolve_control_caps(record: dict[str, Any]) -> dict[str, Any]:
    ...
    if not caps.get("z2m_property"):
        caps["z2m_property"] = mqtt_parse._extract_z2m_property_from_template(
...
def _build_command(
    ...
    if not z2m_prop:
        z2m_prop = mqtt_parse._extract_z2m_property_from_template(value_template)
...
def _entities_from_z2m_exposes(
    ...
    friendly = mqtt_parse._resolve_z2m_display_name(device)
    ...
    state_payload = mqtt_parse._live_z2m_payload(
        states,
        ieee or friendly,
        meta,
            ...
            for d in z2m_devices:
                ...
                try:
                    await client.publish(
                        ...
                        json.dumps(mqtt_parse.z2m_get_payload_for_device(d), ensure_ascii=False),
                        qos=0,
                        retain=False,
                    )
                except Exception:
