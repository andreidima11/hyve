...
async def ensure_client() -> OpenMeteoClient | None:
    cfg = settings.CFG.get("open_meteo") or {}
    entries = _config_location_entries(cfg)
    if entries:
        entry = entries[0]
        return OpenMeteoClient(
            location_query=str(entry.get("location") or "").strip(),
            latitude=_parse_coordinate(entry.get("latitude")),
            longitude=_parse_coordinate(entry.get("longitude")),
            forecast_days=int(entry.get("forecast_days") or 5),
            cache_ttl=int(entry.get("scan_interval") or 900),
            temperature_unit=str(entry.get("temperature_unit") or "celsius"),
            wind_speed_unit=str(entry.get("wind_speed_unit") or "kmh"),
            precipitation_unit=str(entry.get("precipitation_unit") or "mm"),
        )

    location = str(cfg.get("location") or "").strip()
    latitude = cfg.get("latitude")
    longitude = cfg.get("longitude")
    if not location and (latitude in (None, "") or longitude in (None, "")):
        return None
    ...
    return OpenMeteoClient(
        ...
        forecast_days=int(cfg.get("forecast_days") or 5),
        cache_ttl=int(cfg.get("scan_interval") or 900),
        temperature_unit=str(cfg.get("temperature_unit") or "celsius"),
        wind_speed_unit=str(cfg.get("wind_speed_unit") or "kmh"),
        precipitation_unit=str(cfg.get("precipitation_unit") or "mm"),
    )
...
async def ensure_clients() -> list[tuple[dict[str, Any], OpenMeteoClient]]:
    cfg = settings.CFG.get("open_meteo") or {}
    clients: list[tuple[dict[str, Any], OpenMeteoClient]] = []
    for entry in _config_location_entries(cfg):
        latitude = _parse_coordinate(entry.get("latitude"))
        longitude = _parse_coordinate(entry.get("longitude"))
        client = OpenMeteoClient(
            ...
            latitude=latitude,
            longitude=longitude,
            forecast_days=int(entry.get("forecast_days") or cfg.get("forecast_days") or 5),
            cache_ttl=int(entry.get("scan_interval") or cfg.get("scan_interval") or 900),
            temperature_unit=str(entry.get("temperature_unit") or cfg.get("temperature_unit") or "celsius"),
            wind_speed_unit=str(entry.get("wind_speed_unit") or cfg.get("wind_speed_unit") or "kmh"),
            precipitation_unit=str(entry.get("precipitation_unit") or cfg.get("precipitation_unit") or "mm"),
        )
        clients.append((entry, client))
    return clients


async def fetch_all_locations(force: bool = False) -> dict[str, Any]:
    clients = await ensure_clients()
    if not clients:
        return {}

    generated_at = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(time.time()))
    payloads: list[dict[str, Any]] = []
    for index, (entry, client) in enumerate(clients):
        data = await client.fetch_all(force=force)
        payload = dict(data or {})
        location = dict(payload.get("location") or {})
        weather = dict(payload.get("weather") or {})
        entity_id = _entity_id_for_entry(index, entry)
        weather["entity_id"] = entity_id
        weather["source"] = "open_meteo"
        weather["friendly_name"] = str(weather.get("friendly_name") or location.get("name") or weather.get("name") or "Weather").strip()
    ...
    return {
        ...
        "weather": primary.get("weather") or {},
        "locations": payloads,
