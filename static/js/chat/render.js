        ...
        if (copyBtn) {
            copyBtn.addEventListener('click', () => {
                ...
                navigator.clipboard.writeText(txt).then(() => {
                    ...
                    setTimeout(() => { const ic = copyBtn.querySelector('i'); if (ic)
                        ic.className = 'fas fa-copy'; }, 1500);
                }).catch(() => { });
            });
        }
...
export function appendConsciousnessFeedbackBar(bubble, bubbleId, stats) {
    if (!bubble || bubble.querySelector('.chat-intelligence-bar'))
        return;
    ...
    regenBtn.addEventListener('click', () => {
        ...
        while (prev && !prev.classList.contains('chat-row-user'))
