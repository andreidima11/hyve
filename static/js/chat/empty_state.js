...
function greetingItems(key) {
    const raw = tRaw(key);
    if (!Array.isArray(raw))
        return [];
    return raw.filter((item) => !!item && typeof item === 'object' && typeof item.text === 'string');
}
...
function applyRandomGreeting(container) {
    ...
    try {
        ...
        if (Array.isArray(cached.greetings) && cached.greetings.length)
            aiPool = cached.greetings;
    }
    catch (_) { }
