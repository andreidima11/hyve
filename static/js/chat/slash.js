...
function showSlashAutocomplete(filtered) {
    ...
    popup.innerHTML = filtered.map((c, i) => `<div class="slash-autocomplete-item${i === _slashSelectedIdx ? ' selected' : ''}" data-command="${escapeHtml(c.command)}">
...
function hideSlashAutocomplete() {
    if (_slashPopup)
        _slashPopup.classList.add('hidden');
    _slashFiltered = [];
    _slashSelectedIdx = -1;
}
...
export function handleSlashKeydown(e) {
    if (!_slashPopup || _slashPopup.classList.contains('hidden') || !_slashFiltered.length)
        return false;
    if (e.key === 'ArrowDown') {
        e.preventDefault();
