 ...
 */
export function stripThinkFromContent(text) {
...
export function contentAfterThink(text) {
    if (!text || typeof text !== 'string')
        return text || '';
...
export function splitThinkingFromReply(text) {
    if (!text || text.length < 80)
        return null;
    const replyStarters = /(?:\.|\n)\s*(Uite|Iată|So,|Well,|Here'?s?|I'm |Deci,|Așadar,|În concluzie,|Pe scurt,)/gi;
    let lastMatch = null;
    let m;
    while ((m = replyStarters.exec(text)) !== null)
        lastMatch = m;
    if (lastMatch && lastMatch.index >= 50) {
        ...
        if (reply.length > 0 && thinking.length >= 50)
