...
import { clearAttachedDocument, clearAttachedImage, getAttachedDocumentText, getAttachedImageBase64, getAttachedImageDataUrl, getAttachedDocumentFileName, waitForImageReady, } from './attachments.js';
...
import { clearStreamAbortController, finalizeStoppedStreamingBubble, getStreamAbortController, setSendButtonState, setStreamAbortController, } from './stream_control.js';
...
export async function sendMessage(optionalMessage) {
    ...
    if (!msg && !imageBase64 && !documentText)
        return;
    // ── Slash command intercept ─────────────────────────────────
    if (msg.startsWith('/') && !imageBase64 && !documentText) {
        return handleSlashCommand(msg);
    }
    if (input && (typeof optionalMessage !== 'string' || !optionalMessage.trim()))
        input.value = '';
    if (input) {
    ...
    if (input)
        input.blur(); // dismiss keyboard (e.g. Android WebView) after send
    ...
    try {
        ...
        if (bubbleEl && profileColor) {
            ...
            if (row)
                row.style.setProperty('--bubble-glow-color', c);
