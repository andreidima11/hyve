...
const _tts = {
    ...
    _setIndicator(speaking) {
        ...
        if (ind)
            ind.classList.toggle('hidden', !speaking);
        if (btn)
    ...
    async speak(bubble, { fromOffset = 0 } = {}) {
        if (!this._isPiperOn()) {
            console.warn('[TTS] piper not enabled');
            return null;
        }
        if (!bubble)
            return null;
    ...
    async _streamPlayNext(runId = this._streamRunId) {
        ...
        try {
            ...
            if (!res.ok)
                throw new Error('TTS stream fail');
            const blob = await res.blob();
            const url = URL.createObjectURL(blob);
            const audio = new Audio(url);
            audio._blobUrl = url;
