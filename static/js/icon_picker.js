...
export function attachIconPicker(input) {
    if (!input || input.dataset.iconPickerBound === '1')
        return;
...
export function initIconPicker() {
    ...
    document.addEventListener('mousedown', (ev) => {
        ...
        if (pop.contains(ev.target))
            return;
        if (_activeInput && _activeInput.contains(ev.target))
            return;
        _hidePopover();
    });
    const reposition = () => {
        const pop = document.getElementById('icon-picker-popover');
        if (pop && pop.style.display !== 'none' && _activeInput)
            _positionPopover(pop, _activeInput);
    };
