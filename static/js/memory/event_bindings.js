...
function _syncAutomationOpts(el) {
    ...
    if (rerender === 'triggers')
        return { rerenderTriggers: true };
    if (rerender === 'conditions')
        return { rerenderConditions: true };
    if (rerender === 'actions')
        return { rerenderActions: true };
    return {};
}
function _run(action, el, event) {
    if (!_handlers || !_inMemoryScope(el))
        return;
    switch (action) {
        ...
        case 'runAutomationDefinition':
            ...
            if (el.dataset.memoryCloseMenu === 'true')
        ...
        case 'toggleAutomationDefinition':
            _handlers.toggleAutomationDefinition?.(el.dataset.memoryDefId || '', el.dataset.memoryEnabled === 'true', Number(el.dataset.memoryRevision || 1), event, el);
            if (el.dataset.memoryCloseMenu === 'true')
                _handlers.closeAutoMenu?.(event, el);
            return;
...
function _onInput(event) {
    ...
    if (!kind || !_handlers)
        return;
    if (kind === 'filterMemory')
        _handlers.filterMemory?.(event, el);
    ...
    else if (kind === 'updateBlueprintCreatorYaml')
...
function _onChange(event) {
    ...
    if (!kind || !_handlers)
        return;
    if (kind === 'toggleAllMem')
        _handlers.toggleAllMem?.(el.checked, event, el);
    else if (kind === 'syncAutomationYamlFromBuilder') {
        _handlers.syncAutomationYamlFromBuilder?.(_syncAutomationOpts(el), event, el);
    }
        ...
        _handlers.updateBlueprintCreatorYaml?.(event, el);
    else if (kind === 'updateMemBulkCount')
