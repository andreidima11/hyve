...
export function renderMemoryEventsTable(events) {
...
export function updateMemLogPagination() {
    ...
    if (rangeEl)
        rangeEl.textContent = memoryState.logTotal === 0 ? '' : `${from}–${to} of ${memoryState.logTotal}`;
    if (prevBtn)
        prevBtn.disabled = memoryState.logOffset <= 0;
    if (nextBtn)
        nextBtn.disabled = memoryState.logOffset + MEM_LOG_PAGE_SIZE >= memoryState.logTotal;
}
export function formatLearnedTime(ts) {
    if (!ts)
        return '—';
...
export function formatMemoryDate(ts) {
    ...
    let age = '';
    if (days === 0)
        age = formatLearnedTime(ts);
    else if (days === 1)
        age = t('memory.saved_1_day_ago');
    else if (days < 30)
        age = (t('memory.saved_days_ago')).replace('{n}', String(days));
    else
        age = (t('memory.saved_old'));
...
export function renderMemoryTable() {
    ...
    if (memoryState.page > maxPage)
        memoryState.page = maxPage;
    ...
    if (memPrev)
        memPrev.disabled = memoryState.page <= 1;
    if (memNext)
        memNext.disabled = memoryState.page >= maxPage;
