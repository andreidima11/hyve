...
export async function deleteAutomation(jobId: string) {
    ...
    try {
        ...
        if (automationState.editorId === jobId) closeAutomationEditor();
...
export async function validateAutomationEditor(): Promise<void> {
    ...
    try {
        ...
        if (errMsg(e) && !errMsg(e).startsWith('{')) detail = errMsg(e);
...
export async function runAutomationDefinition(automationId: string) {
    try {
        ...
        if (automationState.editorId === automationId) await loadAutomationEditorHistory(automationId);
