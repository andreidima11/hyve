...
import { escapeHtml, setCodeEditorValue, getCodeEditorValue, refreshCodeEditor } from '../utils.js';
...
import { formatAutomationHistoryAt, automationRunStatusBadge, } from './format.js';
import { automationState, automationIdString, editorAutomationId, AUTO_HISTORY_PAGE_SIZE, } from './state.js';
async function _automationLoadCapabilities({ force = false } = {}) {
    if (!force && automationState.capabilities)
        return automationState.capabilities;
    if (!force && automationState.capabilitiesPromise)
...
function _renderAutoHistoryPage() {
    ...
    if (automationState.historyPage > totalPages)
        automationState.historyPage = totalPages;
    if (automationState.historyPage < 1)
        automationState.historyPage = 1;
    ...
    listEl.querySelectorAll('[data-auto-hist-page]').forEach(btn => {
        btn.addEventListener('click', () => {
            automationState.historyPage = Number(btn.dataset.autoHistPage) || 1;
            _renderAutoHistoryPage();
        });
    });
}
...
export function autoSyncAutomationId() {
    if (automationState.idManuallyEdited)
        return;
...
export function removeAutomationBuilderTrigger(idx) {
    ...
    if (!automationState.builderTriggers.length)
        automationState.builderTriggers.push(_automationBuilderTriggerTemplate('time'));
    _automationRenderBuilderTriggers();
    syncAutomationYamlFromBuilder({ silent: true });
}
...
export function removeAutomationBuilderAction(idx) {
    ...
    if (!automationState.builderActions.length)
        automationState.builderActions.push(_automationBuilderActionTemplate('notify'));
    _automationRenderBuilderActions();
    syncAutomationYamlFromBuilder({ silent: true });
}
...
export { _automationLoadCapabilities, _automationResetBuilder, _automationSetEditorMode, _automationHydrateBuilderFromNormalized, _buildAutomationTemplate, _upgradeAutoBuilderSelects, _automationReadBuilderActionsFromDom, _automationRenderBuilderTriggers, _automationRenderBuilderConditions, _automationRenderBuilderActions, _automationBuilderTriggerTemplate, _automationBuilderConditionTemplate, _automationBuilderActionTemplate, };
