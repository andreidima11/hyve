...
function _persistRememberedSession(username: string, accessToken: string, refreshToken?: string): void {
    try {
...
function _pushTokenToNative(token: string): void {
...
export async function handleLogin(e: SubmitEvent): Promise<void> {
    e.preventDefault();
    const user = (document.getElementById('login-user') as HTMLInputElement | null)?.value || '';
    const pass = (document.getElementById('login-pass') as HTMLInputElement | null)?.value || '';
    const remember = (document.getElementById('login-remember') as HTMLInputElement | null)?.checked;
    const err = document.getElementById('login-error');
    const btn = (e.target as HTMLElement | null)?.querySelector('button') as HTMLButtonElement | null;
    void remember;
...
export function restoreRememberedCredentials(): void {
    try {
        const raw = localStorage.getItem(_RM_KEY);
        if (!raw) return;
        const data = JSON.parse(raw) as RememberCredentialsPayload;
        const userEl = document.getElementById('login-user') as HTMLInputElement | null;
        const remEl = document.getElementById('login-remember') as HTMLInputElement | null;
...
export async function tryAutoLogin(): Promise<boolean> {
    try {
        if (await refreshSession()) {
            ...
            if (access) _pushTokenToNative(access);
            return true;
        }
        const raw = localStorage.getItem(_RM_KEY);
        if (!raw) return false;
...
export async function loadUserProfile(): Promise<UserProfileResponse | undefined> {
    try {
        ...
        if (res.ok) {
            ...
            if (displayNameEl && userProfile.username) displayNameEl.innerText = userProfile.username;
            ...
            if (profileUserEl && userProfile.username) profileUserEl.innerText = userProfile.username;
...
export async function handleLinkWhatsApp(e: SubmitEvent): Promise<void> {
    e.preventDefault();
    const phone = (document.getElementById('wa-phone-input') as HTMLInputElement | null)?.value;
    if (!phone) return;
    const res = await apiCall('/api/users/link-whatsapp', { method: 'POST', body: { phone_number: phone } });
    if (res.ok) { void loadUserProfile(); }
}
