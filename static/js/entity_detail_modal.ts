...
export type EntityDetailModalOptions = {
    slug?: string;
    onRegistryUpdated?: (args: { oldEntityId: string; newEntityId: string; uniqueId: string }) => void;
};

export function resolveEntityControlSlug(entity: HyveEntity): string {
    const source = String(entity.source || '').trim();
    if (!source || source === 'derived') return '';
    return integrationIdForSourceSlug(source) || source;
}

export function openEntityDetailModal(entity: HyveEntity, options: EntityDetailModalOptions = {}): void {
    ...
    if (labelEl) labelEl.textContent = entityDisplayName(entity) || String(entity.entity_id || 'Entity');
...
export function closeEntityDetailModal(): void {
    const modal = document.getElementById('entity-detail-modal');
    stopCameraPreviewRefresh();
    pauseEntityDetailCameraStreams(modal);
    if (modal) {
