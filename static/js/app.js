...
window.addEventListener('DOMContentLoaded', () => {
    ...
    initConfigEventBindings({
        ...
        verifyBackup: (_event, el) => verifyBackup(el.dataset.configPath || ''),
        restoreBackup: (_event, el) => restoreBackup(el.dataset.configPath || ''),
        rollbackBackup: (_event, el) => rollbackBackup(el.dataset.configPath || ''),
        saveBackupSettings: () => saveBackupSettings(),
        deleteBackupArchive: (_event, el) => deleteBackupArchive(el.dataset.configPath || ''),
        testBackupRemote: () => testBackupRemote(),
        loadRemoteBackupArchives: () => loadRemoteBackupArchives(),
        pullRemoteBackup: (_event, el) => pullRemoteBackup(el.dataset.configName || ''),
        restoreRemoteBackup: (_event, el) => restoreRemoteBackup(el.dataset.configName || ''),
        downloadBackupArchive: (_event, el) => downloadBackupArchive(el.dataset.configPath || ''),
    ...
    initSmarthomeEventBindings({
        ...
        renameDeviceDetail: (_event, el) => {
            if (!(el instanceof HTMLElement))
                return;
            void renameIntegrationDevice(el.dataset.smarthomeSourceSlug || '', el.dataset.smarthomeDeviceId || '', el.dataset.smarthomeDeviceName || '');
        },
