...
export type SourceIconMeta = { icon: string; color: string; label: string };
...
function _domain(entity: SmarthomeEntity): string {
...
function _iconClass(spec: unknown): string {
...
function _entityIsActive(entity: SmarthomeEntity): boolean {
    const lower = _norm(entity.state);
    return isActiveState(lower) || lower === 'on' || lower === 'true';
}

function _entityIsUnavailable(entity: SmarthomeEntity): boolean {
    const lower = _norm(entity.state);
    return ['unavailable', 'unknown', 'offline'].includes(lower);
}

function _entityIcon(entity: SmarthomeEntity, active = true): string {
    const custom = _iconClass((entity as SmarthomeEntity & { icon?: string }).icon);
...
function _toggleHtml(entity: SmarthomeEntity, extraClass = ''): string {
    const source = String(entity.source || '').trim();
    if (!source || source === 'derived') return '';
    const dom = _domain(entity);
    const eid = String(entity.entity_id || '');
    if (!eid || entity.controllable === false) return '';
    if (!(CONTROLLABLE as readonly string[]).includes(dom)) return '';
    const lower = _norm(entity.state);
    ...
    if (!['switch', 'light', 'input_boolean', 'fan', 'outlet', 'plug'].includes(dom)) return '';
...
function _stateReadout(entity: SmarthomeEntity): string {
    const dom = _domain(entity);
    const lower = _norm(entity.state);
    if (['unavailable', 'unknown', 'offline'].includes(lower)) return tState('unavailable');
    if (dom === 'light') {
...
function _readoutValueClass(text: string): string {
    const len = String(text || '').trim().length;
    const base = 'hyd-hero-readout__value';
    if (len >= 14) return `${base} ${base}--tight`;
    if (len >= 9) return `${base} ${base}--compact`;
    return base;
}

function _syncReadoutValueEl(el: Element | null, text: string): void {
    if (!(el instanceof HTMLElement)) return;
    el.className = _readoutValueClass(text);
    el.textContent = text;
}

function _readoutValueMarkup(text: string): string {
    const raw = String(text || '');
    return `<strong class="${_readoutValueClass(raw)}">${escapeHtml(raw)}</strong>`;
}

function _entityCanToggle(entity: SmarthomeEntity): boolean {
    const source = String(entity.source || '').trim();
    if (!source || source === 'derived') return false;
    const dom = _domain(entity);
    const eid = String(entity.entity_id || '');
    if (!eid || entity.controllable === false) return false;
    if (!(CONTROLLABLE as readonly string[]).includes(dom)) return false;
    return ['switch', 'light', 'input_boolean', 'fan', 'outlet', 'plug'].includes(dom);
}

function _entityDisplayName(entity: SmarthomeEntity): string {
    return String(entity.name || entity.entity_id || '').trim();
}

function _statusControlMarkup(primary: SmarthomeEntity | null): string {
    if (!primary || !_entityCanToggle(primary)) return '';
...
function _renderEntitiesInfoRow(entityCount: number): string {
    return `<div class="hyd-info-row"><dt>${escapeHtml(t('hy.detail_entities'))}</dt><dd>${escapeHtml(String(entityCount))}</dd></div>`;
}

function _entityIconHtml(entity: SmarthomeEntity, size: 'list' | 'hero' = 'list'): string {
    const dom = _domain(entity);
...
function _lightAttr(entityId: string, source: string, kind: string, extra = ''): string {
...
export function entityMatchesCategory(entity: SmarthomeEntity, category: string): boolean {
    const cat = String(category || 'all').toLowerCase();
    if (cat === 'all') return true;
    const dom = _domain(entity);
    if (cat === 'light' || cat === 'lights') return dom === 'light';
    if (cat === 'climate') return ['climate', 'water_heater', 'fan'].includes(dom);
    if (cat === 'sensor' || cat === 'sensors') return dom === 'sensor' || dom === 'binary_sensor';
    if (cat === 'switch' || cat === 'switches') return ['switch', 'input_boolean', 'outlet', 'plug'].includes(dom);
    if (cat === 'camera' || cat === 'cameras') return dom === 'camera';
...
export function renderEntityCategoryTabs(active: string): string {
    const tabs = [
...
function _deviceSummaryReadout(device: PhysicalDeviceGroup): string {
    const primary = primaryDeviceEntity(device);
    if (primary) return _stateReadout(primary);
    const ents = device.entities || [];
    const on = ents.filter((e) => {
        const s = String(e.state || '').toLowerCase();
        return isActiveState(s) || s === 'on';
    }).length;
    if (on) return `${on}/${ents.length} ON`;
    return `${ents.length} ${t('hy.detail_entities')}`;
}

function _deviceIconHtml(device: PhysicalDeviceGroup, size: 'list' | 'hero' = 'list'): string {
    const primary = primaryDeviceEntity(device);
    if (primary) return _entityIconHtml(primary, size);
...
function _rowKind(device: PhysicalDeviceGroup): string {
    const dom = String(device.primary_domain || '').toLowerCase();
    if (dom === 'binary_sensor') return 'sensor';
    if (dom === 'water_heater' || dom === 'fan') return 'climate';
    if (dom && DOMAIN_GLOW[dom]) return dom;
    return 'default';
}

function _rowAvatarIcon(device: PhysicalDeviceGroup): string {
    const primary = primaryDeviceEntity(device);
    if (primary) {
        ...
        const dc = String(caps.device_class || attrs.device_class || '');
        const icon = getDomainIcon(dom, dc);
        const custom = _iconClass((primary as SmarthomeEntity & { icon?: string }).icon);
...
function _rowSubtitle(device: PhysicalDeviceGroup, area: string, srcMeta: SourceIconMeta | null): string {
    if (area) return area;
    if (device.entry_title) return String(device.entry_title);
    if (srcMeta?.label) return srcMeta.label;
    const n = (device.entities || []).length;
    return `${n} ${t('hy.detail_entities')}`;
}

function _rowBrightnessInline(entity: SmarthomeEntity, isOn: boolean): string {
    const source = String(entity.source || '');
    if (!source || source === 'derived') return '';
    const flags = resolveLightControlFlags(entity, isOn);
    if (!flags.hasBrightness) return '';
    const pct = Math.round((flags.brightnessValue / flags.brightnessScale) * 100);
    const eid = String(entity.entity_id || '');
...
function _rowControlsHtml(primary: SmarthomeEntity | null, device: PhysicalDeviceGroup): string {
    if (!primary) {
...
function _editablePageTitleShell(
    viewDataAttr: string,
    viewInnerHtml: string,
    editPanelHtml: string,
): string {
...
function _editablePageTitleEditPanel(
    inputAttrs: string,
    saveAttrs: string,
    cancelAttrs: string,
    panelAttr: string,
...
export function renderDeviceListCard(
    device: PhysicalDeviceGroup,
    sourceIcons: Record<string, SourceIconMeta>,
): string {
    const primary = primaryDeviceEntity(device);
...
export function renderDeviceDetailPage(
    device: PhysicalDeviceGroup,
    sourceIcons: Record<string, SourceIconMeta>,
): string {
    const primary = primaryDeviceEntity(device);
    ...
    const deviceEditPanel = canRename
        ? _editablePageTitleEditPanel(
            `data-device-name-input value="${escapeHtmlAttr(rawName)}"`,
            'data-device-name-save',
            'data-device-name-cancel',
            'data-device-name-edit-panel',
            t('common.save'),
            t('common.cancel'),
        )
        : '';
...
export function renderEntityListCard(
...
function _deviceOverviewCard(
    device: PhysicalDeviceGroup,
    primary: SmarthomeEntity | null,
    sourceIcons: Record<string, SourceIconMeta>,
    ...
    const infoHtml = infoRows.map(([k, v]) =>
        `<div class="hyd-info-row"><dt>${escapeHtml(String(k))}</dt><dd>${escapeHtml(String(v))}</dd></div>`,
    ).join('');
...
export function patchDeviceOverviewDom(device: PhysicalDeviceGroup): void {
    ...
    if (primary) {
        ...
        if (readout) _syncReadoutValueEl(readout, _stateReadout(primary));
        const iconWrap = root.querySelector('.hyd-hero-icon-wrap');
        if (iconWrap) iconWrap.innerHTML = _entityIconHtml(primary, 'hero');
...
export function patchEntityOverviewDom(entity: SmarthomeEntity): void {
    const eid = String(entity.entity_id || '');
    ...
    const readout = root.querySelector('.hyd-hero-readout__value');
    if (readout) _syncReadoutValueEl(readout, _stateReadout(entity));
    const iconWrap = root.querySelector('.hyd-hero-icon-wrap');
    if (iconWrap) iconWrap.innerHTML = _entityIconHtml(entity, 'hero');
    if (overview) {
        const isOn = _entityIsActive(entity);
        const isUnavail = _entityIsUnavailable(entity);
        overview.classList.toggle('is-on', isOn);
    ...
    if (dom === 'light') {
        ...
        if (flags.hasBrightness) {
            const pct = Math.round((flags.brightnessValue / flags.brightnessScale) * 100);
            const range = root.querySelector('.hyd-overview__brightness .hyd-brightness__range') as HTMLInputElement | null;
...
export function patchEntityDetailDom(entity: SmarthomeEntity): void {
    patchEntityOverviewDom(entity);
    const eid = String(entity.entity_id || '');
    const root = document.querySelector(`[data-entity-detail-id="${CSS.escape(eid)}"]`);
    if (!root) return;
    const aiCheck = root.querySelector('.hyd-ai-check') as HTMLInputElement | null;
    if (aiCheck) aiCheck.checked = !!entity.selected;
}

function _entityAuxControlsHtml(entity: SmarthomeEntity): string {
...
export function renderEntityDetailPage(
    ...
    const canEditName = !!(entity.unique_id || (entity.attributes as Record<string, unknown> | undefined)?.registry_unique_id || (entity.attributes as Record<string, unknown> | undefined)?.unique_id);
    ...
    const entityEditPanel = canEditName
        ? _editablePageTitleEditPanel(
            `data-entity-friendly-name-input value="${escapeHtmlAttr(entity.name || entityId)}"`,
            'data-entity-friendly-name-save',
            'data-entity-friendly-name-cancel',
            'data-entity-friendly-name-edit-panel',
            t('common.save'),
            t('common.cancel'),
        )
        : '';
