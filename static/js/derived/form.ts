...
export function setBuilderUi(builder: BuilderKind) {
    ...
    if (builder === BUILDER_TRANSFORM && derivedState.selectedInputs.size > 1) {
        const first = derivedState.selectedInputs.values().next().value as string;
...
export function setViewUi(view: ViewKind) {
    ...
    if (view === VIEW_YAML) {
        ...
        if (shouldSync) renderYamlFromForm();
        updateYamlSyncBadge();
    }
    schedulePreview();
}
...
function updateInputsCount() {
    ...
    if (el) el.textContent = String(derivedState.selectedInputs.size);
}
...
export function toggleDerivedInput(el: HTMLInputElement) {
    if (derivedState.builder === BUILDER_TRANSFORM) {
        ...
        if (el.checked) derivedState.selectedInputs.add(el.value);
        else derivedState.selectedInputs.delete(el.value);
        updateInputsCount();
    }
    schedulePreview();
    syncYamlIfUntouched();
...
export function buildFormulaPayload(): DerivedFormula {
...
function buildEntryPayload(): DerivedEntry {
    return {
...
export async function renderYamlFromForm() {
    ...
    try {
        const res = await apiCall('/api/derived/yaml/serialize', {
            method: 'POST',
            body: buildEntryPayload() as Record<string, unknown>,
        });
        if (res.ok) {
...
export function syncYamlIfUntouched() {
    if (derivedState.view !== VIEW_YAML) return;
    if (derivedState.yamlTouched) return;
    renderYamlFromForm();
}
...
export function schedulePreview() {
    if (derivedState.previewTimer) clearTimeout(derivedState.previewTimer);
    derivedState.previewTimer = setTimeout(runDerivedPreview, 400);
}

async function resolveFormulaForPreview(): Promise<DerivedPreviewResolved | null> {
...
function setPreviewInputs(html: string) {
...
export async function runDerivedPreview() {
    ...
    if (formula.type === 'expression' && !('expression' in formula && formula.expression)) {
        valueEl.textContent = '—'; setPreviewInputs(''); return;
    }
