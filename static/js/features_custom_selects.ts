...
interface PortaledSelectMenu extends HTMLDivElement {
    __placeholder?: Comment | null;
    __ownerDd?: GenericCustomSelectElement | null;
}

interface GenericCustomSelectElement extends HTMLElement {
    __portaledMenu?: PortaledSelectMenu | null;
}

interface UpgradableNativeSelect extends HTMLSelectElement {
...
function _rebuildGenericSelect(dd: GenericCustomSelectElement) {
...
export function initGenericCustomSelects(root?: ParentNode) {
    ...
    scope.querySelectorAll('.dashboard-custom-select.js-generic-select[data-target]').forEach((dd) => {
        _rebuildGenericSelect(dd as GenericCustomSelectElement);
    });
}
...
export function upgradeNativeSelect(sel: HTMLSelectElement) {
    ...
    if (nextEl && nextEl.classList.contains('js-generic-select') && nextEl.getAttribute('data-target') === sel.id) {
        _rebuildGenericSelect(nextEl as GenericCustomSelectElement);
        return;
    }
    ...
    if (!sel._optObserver && typeof MutationObserver !== 'undefined') {
        sel._optObserver = new MutationObserver(() => {
            ...
            if (ov && ov.classList.contains('js-generic-select')) _rebuildGenericSelect(ov as GenericCustomSelectElement);
        });
    ...
    document.addEventListener('change', (e) => {
        ...
        if (sel instanceof HTMLSelectElement && sel.classList.contains('dashboard-custom-select-native')) {
            ...
            if (overlay && overlay.classList.contains('js-generic-select')) {
                _rebuildGenericSelect(overlay as GenericCustomSelectElement);
            }
        }
    }, true);
    let _pending = false;
