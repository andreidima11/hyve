...
let _integrationEntitiesCache: SmarthomeEntity[] = [];
let _devicesVisibleEntityCache = new Map<string, SmarthomeEntity>();
let _smarthomeLoadPromise: Promise<void> | null = null;
let _smarthomeLoadRetryTimer: ReturnType<typeof setTimeout> | null = null;
let _deviceControlPending = new Map<string, { action: string; previousState: unknown; optimisticState: unknown; startedAt: number }>();
let _deviceOptimisticGuards = new Map<string, { state: unknown; until: number }>();
...
const _devicesState: DevicesState = {
    query: '',
    source: 'all',
    area: 'all',
    domain: 'all',
...
function _iconClass(spec: unknown) {
...
function _entityDomain(entity: SmarthomeEntity | null | undefined) {
...
function _syncDevicesStateFromInputs({ resetPage = false }: { resetPage?: boolean } = {}) {
    _devicesState.query = _norm((document.getElementById('hy-search') as HTMLInputElement | null)?.value || '');
...
let _smarthomeLiveWS: WebSocket | null = null;
let _smarthomeLiveReconnectTimer: ReturnType<typeof setTimeout> | null = null;
let _smarthomeLivePingTimer: ReturnType<typeof setInterval> | null = null;
...
function _optimisticStateForAction(action: string, currentState: unknown) {
...
function _markDeviceControlPending(entityId: string, pending: boolean) {
...
function _getFilteredDevices() {
    ...
    const filtered = allDevices.filter(entity => {
        ...
        if (domainFilter === 'active') {
            if (!_isActiveState(_norm(entity.state))) return false;
...
export function toggleHABulkMode() {
    ...
    if (!_haBulkMode) {
        document.querySelectorAll('.hy-bulk-check').forEach(cb => { (cb as HTMLInputElement).checked = false; });
        const allCheck = document.getElementById('hy-select-all');
        if (allCheck) (allCheck as HTMLInputElement).checked = false;
        updateHABulkCount();
    }
...
const SOURCE_ICONS: Record<string, SourceIconMeta> = {
...
function renderDeviceCards() {
    ...
    tbody.innerHTML = pageDevices.map(entity => {
        ...
        const customIconCls = _iconClass((entity as SmarthomeEntity & { icon?: string }).icon);
...
function _domainLabel(domain: string) {
...
export function toggleAllHA(checked: boolean) {
    document.querySelectorAll('.hy-bulk-check').forEach(cb => { (cb as HTMLInputElement).checked = checked; });
    updateHABulkCount();
}
...
export async function deleteHASingle(eid: string) {
    // No-op: single-entity delete was Home Assistant only.
}

export async function toggleDevice(eid: string, btnEl: HTMLElement | null) {
...
export function handleHaRowClick(event: MouseEvent) {
    const row = event.currentTarget as HTMLElement | null;
    if (!row || row.getAttribute('data-entity') == null) return;
    const tgt = event.target as HTMLElement | null;
    if (tgt?.closest('button, input, a, label')) return;
    const eid = row.getAttribute('data-entity');
    if (eid) openRowActionsModal(eid);
}

export async function openRowActionsModal(entityId: string) {
    ...
    const iconClass = _iconClass((entity as SmarthomeEntity & { icon?: string }).icon) || `fas ${DOMAIN_ICONS[domain] || 'fa-microchip'}`;
...
function _wireCameraPreviewMute() {
    ...
    const video = wrap?.querySelector('video[data-camera-live-webm]') as HTMLVideoElement | null;
    const btn = wrap?.querySelector('[data-camera-mute-toggle]') as HTMLButtonElement | null;
    if (!video || !btn) return;
...
function _cameraPreviewMarkup(entity: SmarthomeEntity, attrs: Record<string, unknown>) {
    const domain = _entityDomain(entity);
...
function _imagePreviewMarkup(entity: SmarthomeEntity, attrs: Record<string, unknown>) {
...
function _imageProxyUrl(entityId: string) {
    if (!entityId) return '';
    return cameraProxyUrlSync(entityId, 'image');
}

function _cameraProxyUrl(entityId: string, mode: string = 'snapshot') {
    if (!entityId) return '';
    const paths: Record<string, string> = { stream: 'stream', play: 'play', snapshot: 'snapshot' };
    const path = paths[mode] || 'snapshot';
    return cameraProxyUrlSync(entityId, path);
}

function _cacheBustCameraUrl(url: string) {
...
function _deviceControlButtons(entity: SmarthomeEntity) {
    ...
    const button = (action: string, icon: string, label: string, tone = '') => {
...
export async function controlDeviceEntity(source: string, entityId: string, action: string, buttonEl: HTMLElement | null = null) {
    ...
    entity.state = optimisticState as string | number | null;
    renderDeviceCards();
...
export function openAliasModalFromDetail(entityId: string) {
...
export async function saveAliases(eid: string, val: string) {
