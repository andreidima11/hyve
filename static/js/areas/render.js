...
export function _iconClass(spec, fallback = 'fas fa-location-dot') {
...
export function _entityLookup(eid) {
...
export function _renderPickerList() {
    ...
    if (counter)
        counter.textContent = t('areas.picker_selected', { selected: areaState.pickerSelected.size, filtered: filtered.length, total: areaState.allEntitiesCache.length });
    if (!filtered.length) {
