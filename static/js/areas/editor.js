...
function _openEditor(area) {
    ...
    try {
        ...
        if (titleEl)
            titleEl.innerHTML = `<i class="fas fa-house-chimney-window"></i><span>${render._esc(isEdit ? t('areas.editor_title_edit') : t('areas.editor_title_new'))}</span>`;
        ...
        const nameEl = document.getElementById('area-editor-name');
        const floorEl = document.getElementById('area-editor-floor');
        const iconEl = document.getElementById('area-editor-icon');
        const aliasesEl = document.getElementById('area-editor-aliases');
        if (nameEl)
...
async function _ensureEntitiesLoaded(force = false) {
    const fresh = (Date.now() - areaState.entitiesCacheTime) < 30000;
    if (!force && fresh && areaState.allEntitiesCache.length)
...
export async function openAreaEntityPicker() {
    ...
    if (list)
        list.innerHTML = `<div class="text-center text-xs text-slate-500 py-8"><i class="fas fa-spinner fa-spin mr-1.5"></i>${render._esc(t('areas.loading_entities'))}</div>`;
    try {
        ...
        if (list)
            list.innerHTML = `<div class="text-center text-xs text-rose-400 py-8">${render._esc(t('areas.load_entities_error'))}</div>`;
    }
}
...
export function toggleAreaPickerEntity(eid, checked) {
    if (checked)
        areaState.pickerSelected.add(eid);
    else
        areaState.pickerSelected.delete(eid);
    const counter = document.getElementById('area-entity-picker-count');
    if (counter)
        counter.textContent = t('areas.picker_selected_total', { selected: areaState.pickerSelected.size, total: areaState.allEntitiesCache.length });
}
