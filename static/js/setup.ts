...
function _populateTimezones(preferred?: string) {
    const select = document.getElementById('setup-timezone') as HTMLSelectElement | null;
    if (!select) return;
...
async function _submitSetup() {
    const btn = document.getElementById('setup-next-btn') as HTMLButtonElement | null;
...
async function _handleSetupSubmit(e: Event) {
    e.preventDefault();
    _showError('');
    if (_step === 1) {
...
export function showSetupWizard(status?: SetupStatus) {
    _status = status || {};
    _step = 1;
    const overlay = document.getElementById('setup-overlay');
    if (overlay) {
    ...
    const serverName = document.getElementById('setup-server-name') as HTMLInputElement | null;
