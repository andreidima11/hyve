...
async function findExistingEmptySession(excludeSessionId: string | null = null): Promise<SessionDetail | null> {
    try {
...
function _sessionDeleteWrapHtml(id: string) {
...
function _sessionDeleteButtonHtml(id: string, deleteTip: string) {
    const sessionId = escapeHtml(id || '');
    return `
...
export function deleteSession(id: string, evt?: Event) {
    if (evt) {
        evt.stopPropagation();
        evt.preventDefault();
    }
...
export function cancelDeleteSession(id: string) {
    const row = document.querySelector(`.session-item[data-id="${id}"]`);
    const wrap = row?.querySelector('.session-delete-wrap');
    if (!wrap) return;
...
export async function confirmDeleteSession(id: string) {
    if (!id) return;
    try {
...
export async function clearSessionContext() {
    const clearBtn = document.getElementById('btn-clear-context') as HTMLButtonElement | null;
