...
export function syncIntegrationToggles() {
    ...
    if (alwaysSpeakBtn)
        alwaysSpeakBtn.classList.toggle('hidden', !anyTtsOn);
    ...
    if (voiceBtn) {
        ...
        if (!whisperEnabled) {
            if (voiceBtn.classList.contains('recording')) {
                ...
                if (icon)
                    icon.className = isVoiceLoopActive() ? 'fas fa-sync-alt' : 'fas fa-microphone';
            }
        }
    }
...
function updateIntegrationSubtab() {
    ...
    if (ac)
        ac.textContent = activeCount > 0 ? `(${activeCount})` : '';
    if (avc)
        avc.textContent = availableCount > 0 ? `(${availableCount})` : '';
}
// --- ComfyUI helpers ---
...
export function bindIntegrationToggleButtonsOnce() {
    if (_integrationToggleButtonsBound)
        return;
    ...
    document.addEventListener('click', (e) => {
        ...
        if (!checkbox)
            return;
        if (btn.id.includes('-btn-enable'))
            checkbox.checked = true;
        if (btn.id.includes('-btn-disable'))
...
export async function loadIntegrationCatalog(force = false) {
    if (_integrationCatalog.length && !force)
...
function _entityMeta(key) {
    ...
    if (meta)
        return { icon: meta.icon, label: _ed(meta.labelKey) };
...
function _renderDetailProfil(data) {
    if (!data || data.error)
        return `<span class="text-red-400 text-[10px]">${escapeHtml(_ed('error'))}</span>`;
    return [
...
function _renderDetailVehicule(data) {
    if (!Array.isArray(data) || !data.length)
        return `<span class="text-slate-500 text-[10px]">${escapeHtml(_ed('no_vehicles'))}</span>`;
    const alertLabels = {
    ...
    return data.map(v => {
        ...
        if (rcaDays !== null && rcaDays < 0) {
            status = _ed('vehicle_rca_expired');
            statusCls = 'text-red-400';
        }
        else if (itpDays !== null && itpDays < 0) {
            status = _ed('vehicle_itp_expired');
            statusCls = 'text-red-400';
        }
        else if (!alerte.rca_expira) {
            status = _ed('vehicle_no_rca');
        ...
        for (const [key, labelKey] of Object.entries(alertLabels)) {
            ...
            if (days !== null) {
                if (days < 0) {
                    cls = 'text-red-400';
                    extra = _ed('expired_suffix');
                }
                else {
                    extra = _ed('days_suffix', { days });
                }
            }
            tags.push(`<span class="${cls}">${escapeHtml(_ed(labelKey))} ${dateStr}${escapeHtml(extra)}</span>`);
        }
        const notifs = [];
        if (alerte.rca_notificare_sms)
            notifs.push('SMS');
        if (alerte.rca_notificare_email)
            notifs.push('Email');
...
function _renderDetailFacturi(data) {
    if (!Array.isArray(data) || !data.length)
        return `<span class="text-slate-500 text-[10px]">${escapeHtml(_ed('no_bills'))}</span>`;
...
function _renderDetailFusionSummary(data) {
    if (!data || typeof data !== 'object')
        return `<span class="text-slate-500 text-[10px]">${escapeHtml(_ed('no_data'))}</span>`;
    const rows = [
...
function _renderDetailFusionStations(data) {
    if (!Array.isArray(data) || !data.length)
        return `<span class="text-slate-500 text-[10px]">${escapeHtml(_ed('no_stations'))}</span>`;
...
function _renderDetailFusionRealtime(data) {
    if (!Array.isArray(data) || !data.length)
        return `<span class="text-slate-500 text-[10px]">${escapeHtml(_ed('no_live_data'))}</span>`;
    return data.map((item, i) => {
        const rows = [
...
function _renderDetailFusionYearly(data) {
    if (!Array.isArray(data) || !data.length)
        return `<span class="text-slate-500 text-[10px]">${escapeHtml(_ed('no_yearly_data'))}</span>`;
    return data.map((item, i) => {
...
function _renderDetailFusionDevices(data) {
    if (!Array.isArray(data) || !data.length)
        return `<span class="text-slate-500 text-[10px]">${escapeHtml(_ed('no_devices'))}</span>`;
...
export async function syncIntegrationEntities(slug, options = {}) {
    ...
    if (btn) {
        btn.disabled = true;
        btn.innerHTML = `<i class="fas fa-spinner fa-spin mr-1"></i>${escapeHtml(_ed('sync_btn'))}`;
    }
    try {
        ...
        if (btn) {
            btn.disabled = false;
            btn.innerHTML = `<i class="fas fa-sync-alt mr-1"></i>${escapeHtml(_ed('sync_btn'))}`;
        }
    }
}
;
...
export async function loadIntegrationEntities(slug) {
    ...
    try {
        ...
        if (timeEl && data.updated_at) {
            ...
            const isStale = age > 2 * 3600000; // older than 2h
        ...
        const allEmpty = Object.values(entities).every(v => {
            ...
            if (typeof v === 'object' && v)
                return !!v.error || Object.keys(v).length === 0;
            return true;
        });
        if (allEmpty && errEl) {
