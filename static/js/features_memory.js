...
function updateMemLogPagination() {
    ...
    if (rangeEl)
        rangeEl.textContent = memLogTotal === 0 ? '' : `${from}–${to} of ${memLogTotal}`;
    if (prevBtn)
        prevBtn.disabled = memLogOffset <= 0;
    if (nextBtn)
        nextBtn.disabled = memLogOffset + MEM_LOG_PAGE_SIZE >= memLogTotal;
}
...
export function memLogNextPage() {
    if (memLogOffset + MEM_LOG_PAGE_SIZE >= memLogTotal)
        return;
...
export async function runConsolidationNow() {
    ...
    if (resultEl)
        resultEl.textContent = t('memory.consolidation_running');
    try {
...
export function setMemLogType(value, label) {
    ...
    if (dd) {
        ...
        if (valueEl)
            valueEl.textContent = label || value || (t('memory.log_type_all'));
...
export function renderMemoryTable() {
    ...
    if (memPrev)
        memPrev.disabled = memPage <= 1;
    if (memNext)
        memNext.disabled = memPage >= maxPage;
