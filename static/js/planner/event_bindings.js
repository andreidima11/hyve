...
let _bound = false;
function _listId(el) {
    const host = el.closest('[data-planner-list-id]');
    return Number(el.dataset.plannerListId || (host instanceof HTMLElement ? host.dataset.plannerListId : '') || 0);
}
function _entryId(el) {
    const host = el.closest('[data-planner-entry-id]');
    return Number(el.dataset.plannerEntryId || (host instanceof HTMLElement ? host.dataset.plannerEntryId : '') || 0);
}
function _run(action, el, event) {
    if (!_handlers || !el.closest('#view-planner'))
        return;
    if (el.dataset.plannerStopPropagation === 'true')
    ...
    switch (action) {
        case 'selectList':
            ...
            if (el.dataset.plannerCloseDrawer === 'true')
                _handlers.closeDrawer?.(event, el);
            return;
...
function _onClick(event) {
    ...
    if (!(target instanceof Element))
        return;
    const el = target.closest('[data-planner-action]');
...
function _onKeydown(event) {
    if (event.key !== 'Enter')
        return;
    const target = event.target;
    if (!(target instanceof HTMLElement) || !target.closest('#view-planner'))
        return;
...
function _onDragStart(event) {
    ...
    if (!(target instanceof Element))
        return;
    const el = target.closest('[data-planner-drag-start]');
    if (!(el instanceof HTMLElement) || !el.closest('#view-planner'))
        return;
    const kind = el.dataset.plannerDragStart;
    const id = _entryId(el);
    if (kind === 'task')
        _handlers?.taskDragStart?.(event, id, el);
    else if (kind === 'event')
        _handlers?.eventDragStart?.(event, id, el);
}
function _onDragOver(event) {
    const target = event.target;
    if (!(target instanceof Element))
        return;
    const el = target.closest('[data-planner-drag-over]');
    if (!(el instanceof HTMLElement) || !el.closest('#view-planner'))
        return;
    const kind = el.dataset.plannerDragOver;
    if (kind === 'task')
        _handlers?.taskDragOver?.(event, el);
    else if (kind === 'event')
        _handlers?.eventDragOver?.(event, el);
}
function _onDrop(event) {
    const target = event.target;
    if (!(target instanceof Element))
        return;
    const el = target.closest('[data-planner-drop]');
    if (!(el instanceof HTMLElement) || !el.closest('#view-planner'))
        return;
    ...
    if (kind === 'eventHour') {
        _handlers?.eventDropHour?.(event, el.dataset.plannerCalDay || '', Number(el.dataset.plannerCalHour ?? 0), el);
    }
}
function _onDragEnd(event) {
    ...
    const id = _entryId(el);
    if (kind === 'task')
        _handlers?.taskDragEnd?.(event, id, el);
    else if (kind === 'event')
        _handlers?.eventDragEnd?.(event, id, el);
}
