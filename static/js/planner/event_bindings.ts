...
function _listId(el: HTMLElement): number {
...
function _entryId(el: HTMLElement): number {
...
function _onDragStart(event: DragEvent): void {
    const target = event.target;
    if (!(target instanceof Element)) return;
    const el = target.closest('[data-planner-drag-start]');
    if (!(el instanceof HTMLElement) || !el.closest('#view-planner')) return;
...
function _onDragOver(event: DragEvent): void {
    const target = event.target;
    if (!(target instanceof Element)) return;
...
function _onDrop(event: DragEvent): void {
    const target = event.target;
    if (!(target instanceof Element)) return;
...
function _onDragEnd(event: DragEvent): void {
    const target = event.target;
    if (!(target instanceof Element)) return;
    const el = target.closest('[data-planner-drag-start]');
    if (!(el instanceof HTMLElement) || !el.closest('#view-planner')) return;
...
export function initPlannerEventBindings(handlers: DelegatedEventHandlers = {}): void {
    _handlers = handlers;
    if (_bound) return;
    _bound = true;
