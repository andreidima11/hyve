...
import { isVoiceLoopActive, toggleVoiceLoopActive, setVoiceInputPending, } from './voice_state.js';
...
export async function toggleVoiceRecording(opts) {
    ...
    if (_voiceMediaRecorder && _voiceMediaRecorder.state === 'recording') {
        ...
        if (_voiceSilenceTimer) {
            cancelAnimationFrame(_voiceSilenceTimer);
        ...
        if (icon)
            icon.className = _voiceMicIconClass();
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        ...
        if (!isSecure)
            showToast(t('voice.requires_https'), 'error', 6000);
        else
            showToast(t('voice.mic_unavailable'), 'error');
        return;
    }
    try {
    ...
    _voiceMediaRecorder.onstop = async () => {
        ...
        try {
            ...
            if (micIcon)
                micIcon.className = _voiceMicIconClass();
        }
    };
...
function _initAlwaysSpeakBtn() {
    ...
    if (!btn)
        return;
    if (btn.dataset.boundAlwaysSpeak === '1')
        return;
    ...
    btn.addEventListener('click', async (e) => {
        ...
        if (ttsCtrl.alwaysSpeak) {
            ...
            if (lastBubble && typeof ttsCtrl.speak === 'function') {
                try {
                    await ttsCtrl.speak(lastBubble);
                }
                catch (err) {
    ...
    function _syncLoopUI() {
        ...
        if (on && tts) {
            ...
            if (asBtn) {
                ...
                if (asIcon)
                    asIcon.className = 'fas fa-volume-up';
            }
        }
    }
