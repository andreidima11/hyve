...
import type { ConfigFormElement, ModelProfile, ModelProfilesResponse, SaveConfigOptions } from './types/features_config.js';

/** Most config DOM ids are form controls; keeps load/save terse. */
function cfgField(id: string): ConfigFormElement | null {
    return document.getElementById(id) as ConfigFormElement | null;
}

function cfgNode(id: string): HTMLElement | null {
    return document.getElementById(id);
}

function _cfgVal(id: string): string {
...
function _integrationSlugCandidates(slug: string): string[] {
...
function formatHealthError(detail: unknown): string {
...
async function _savePiperAddonConfig(): Promise<void> {
...
function _updateSearchTendencyHint(val: number) {
...
function _refreshUiLanguageSelect(language: string) {
...
async function _applyAndSaveUiLanguage(language: string) {
    if (!language) return;
...
function renderUserPhonesList(phones: string[]) {
...
export async function addUserPhone(phone: string, inputEl: ConfigFormElement | null) {
    if (!phone) return;
    try {
...
export async function unlinkUserPhone(number: string) {
...
export async function testPiperConnection() {
    ...
    const setBtnState = (type: 'ok' | 'fail', text: string) => {
