...
function _applyLightControlOptimistic(
    ent: HyveEntity,
    action: string,
...
function _syncLightControlInputs(entityId: string, attrs: Record<string, unknown>): void {
    const eid = CSS.escape(String(entityId));
    if (attrs.color_temp != null) {
        const ct = String(attrs.color_temp);
        document.querySelectorAll<HTMLInputElement>(
            `[data-int-entity-id="${eid}"][data-int-input="color_temp"]`,
        ).forEach((el) => { el.value = ct; });
    ...
    if (attrs.brightness != null) {
        ...
        document.querySelectorAll<HTMLInputElement>(
            `[data-int-entity-id="${eid}"][data-int-input="brightness"]`,
        ).forEach((el) => { el.value = br; });
    }
}
...
function _patchIntegrationExposedEntityState(item: Record<string, unknown>): void {
    ...
    if (_exposedDevicesState?.devices) {
        for (const dev of _exposedDevicesState.devices) {
            ...
            if (ent) {
                if (item.state !== undefined && item.state !== null) ent.state = String(item.state);
                if (item.unit) ent.unit = String(item.unit);
                if (attrPatch) {
                    const merged = { ...((ent.attributes || {}) as Record<string, unknown>) };
...
function _renderEntityControlRow(ent: HyveEntity, slug: string): string {
    ...
    if (dom === 'light') {
        const lightExtras = renderLightControlsMarkup(
            ent,
            slug,
            _intCtrlAttrs,
            escapeHtml,
...
export function renderDeviceEntityListMarkup(ents: HyveEntity[], slug: string, deviceId: string): string {
    return _renderPaginatedEntityList(ents, slug, deviceId);
}

export function wireDeviceEntityList(root: HTMLElement, ents: HyveEntity[], slug: string, deviceId: string): void {
...
export async function controlIntegrationEntity(slug: string, entityId: string, action: string, btn: HTMLElement | null, data: Record<string, unknown> | null) {
    ...
    if (_exposedDevicesState.slug && integrationSlugsMatch(_exposedDevicesState.slug, slug)) {
        ...
        if (touchedEnt) {
            ...
            else if (_LIGHT_FINE_ACTIONS.has(action)) _applyLightControlOptimistic(touchedEnt, action, data);
...
function _patchIntegrationEntityDeviceNames(deviceId: string, name: string) {
    for (const ent of smarthomeDeviceState.integrationEntitiesCache) {
        ...
        if (attrs) attrs.device_name = name;
    }
}

export async function renameIntegrationDevice(
    slug: string,
    deviceId: string,
    currentName: string,
    providedName?: string,
    homeassistantRename = true,
    options: { skipDetailRefresh?: boolean } = {},
): Promise<{ device_id?: string; name?: string } | void> {
    ...
    try {
        ...
        const out = await res.json().catch(() => ({})) as { device_id?: string; name?: string; detail?: unknown };
