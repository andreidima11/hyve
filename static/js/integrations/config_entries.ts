...
export async function loadIntegrationConfigEntries(slug: string) {
    ...
    _entriesCurrent.entries.forEach(entry => {
        ...
        if (deleteBtn) deleteBtn.onclick = async () => {
            ...
            try {
                ...
                if (typeof showToast === 'function') showToast(errMsg(e) || t('common.error'), 'error', 2500);
            }
        };
...
function openEntryEditor(entry: Record<string, unknown> | null): void {
    ...
    const collectData = () => {
        ...
        for (const field of _entriesCurrent.schema) {
            ...
            if (!el) continue;
            let v;
            if (field.type === 'bool' || field.type === 'boolean') v = !!el.checked;
            else if (field.type === 'number') v = el.value === '' ? null : Number(el.value);
            else v = el.value;
            if (editingEntryId && field.secret && typeof v === 'string' && /^[•*]+$/.test(v)) continue;
            out[fkey] = v;
        }
        return out;
    };
    ...
    if (testBtn) {
        testBtn.onclick = async () => {
            ...
            try {
                ...
                errEl.textContent = (e as Error).name === 'TimeoutError'
...
async function _runOAuthConnect(field: Record<string, unknown>, btn: HTMLButtonElement, errEl: HTMLElement, closeModal: () => void) {
    ...
    try {
        ...
        reset();
        if (errEl) { errEl.textContent = errMsg(e) || t('common.error'); errEl.classList.remove('hidden'); }
        return;
    }
