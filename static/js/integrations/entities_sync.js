...
export async function syncIntegrationEntities(slug, options = {}) {
    ...
    if (btn) {
        btn.disabled = true;
        btn.innerHTML = `<i class="fas fa-spinner fa-spin mr-1"></i>${escapeHtml(entityDetailText('sync_btn'))}`;
    }
    try {
        ...
        if (btn) {
            btn.disabled = false;
            btn.innerHTML = `<i class="fas fa-sync-alt mr-1"></i>${escapeHtml(entityDetailText('sync_btn'))}`;
        }
    }
}
;
    ...
    try {
        ...
        for (const [key, value] of Object.entries(entities)) {
            ...
            if (Array.isArray(value))
                ...
                count = entityDetailText('fields_count', { count: Object.keys(value).length });
            else if (value?.error)
                count = entityDetailText('error_badge');
...
function openEntityDetailModal(key, value, meta) {
