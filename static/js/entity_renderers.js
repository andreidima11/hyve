...
function _stateLabel(state, unit = '', domain = '') {
    ...
    window.__hyveCameraFrameFailed = function (img, fallbackSrc = '') {
        ...
        if (frame) {
            ...
            if (loader) {
                ...
                if (label)
                    label.textContent = _er('camera_unavailable');
            }
        }
    };
}
...
export async function saveEntityRegistryId(entity, objectIdInput) {
    ...
    if (nextEntityId === entity.entity_id)
        return { entry: null, entity_id: nextEntityId, unchanged: true };
    const res = await apiCall(`/api/integrations/entities/registry/${encodeURIComponent(uid)}`, { method: 'PATCH', body: { entity_id: nextEntityId } });
...
export function wireEntityRegistryEditor(container, entity, options = {}) {
    ...
    const submit = async () => {
        ...
        try {
            ...
            if (display)
                display.textContent = result.entity_id;
...
function renderSelect(entity, slug) {
    ...
    const attrs = (entity.attributes || {});
    const caps = attrs.capabilities || {};
    const selectHtml = renderSelectControlHtml(slug, eid, attrs, caps, String(entity.state ?? ''), _ctrlAttrs, _attr, escapeHtml);
    if (!selectHtml)
        return '';
    return `
    <div class="rounded-2xl bg-white/5 border border-white/10 p-4 mb-3">
...
function renderCamera(entity, slug) {
    ...
    if (useGo2rtc)
        streamAttrs.push('go2rtc');
    if (transport === 'webm')
        streamAttrs.push('webm');
    if (hasAudio)
        streamAttrs.push('show-mute');
    else
        streamAttrs.push('muted');
    return `
    <div class="hy-entity-camera-shell mb-3">
