...
import { cameraProxyUrlSync, pauseEntityDetailCameraStreams, startCameraPreviewRefresh, stopCameraPreviewRefresh, } from '../camera_auth.js';
...
function _deviceLightControls(entity) {
    ...
    if (hasColor) {
        body += renderHyColorPickerMarkup(colorHex, ctrlAttrs('color'), escapeHtml, escapeHtmlAttr, { color: _er('color'), hue: _er('hue') });
    }
    if (hasColorTemp) {
        body += `
        <div class="hy-detail-light-row">
