 ...
 */
import { autoPrimaryDeviceEntity, getDevicePrimaryEntityOverride, primaryEntityCandidates, } from '../device_primary_entity.js';
...
let _holdStartX = 0;
let _holdStartY = 0;
let _holdMoveListener = null;
let _holdUpListener = null;
...
export function consumeDevicePrimaryHoldClick(root) {
    if (!root || root.dataset.holdFired !== 'true')
        return false;
    delete root.dataset.holdFired;
    return true;
}
function _renderPrimaryEntityModalBody(device) {
...
export function openDevicePrimaryEntityModal(device) {
...
export function closeDevicePrimaryEntityModal() {
    const modal = document.getElementById('hy-device-primary-modal');
    if (!modal)
        return;
...
export function initDevicePrimaryHoldBindings(resolveDevice) {
    if (_holdBound || typeof document === 'undefined')
        return;
    _holdBound = true;
    document.addEventListener('pointerdown', (event) => {
        ...
        if (!(target instanceof Element))
            return;
        const root = target.closest('[data-smarthome-primary-hold-root]');
        if (!root)
            return;
        if (event.button !== 0)
            return;
        ...
        _holdMoveListener = (ev) => _cancelHold(ev);
