...
export function renderDeviceCards() {
    ...
    if (!devices.length) {
        ...
        const filtersActive = smarthomeDeviceState.devicesState.domain !== 'all' || smarthomeDeviceState.devicesState.source !== 'all' || smarthomeDeviceState.devicesState.area !== 'all' || !!smarthomeDeviceState.devicesState.query;
    ...
    smarthomeDeviceState.devicesVisibleEntityCache = new Map(pageDevices.map(entity => [_entityId(entity), entity]));
...
function _renderDevicesPagination(total, from, to, totalPages) {
    ...
    return `<div class="hy-devices-pager-info">
        ...
        <div class="hy-devices-pager-actions">
            ...
            <button type="button" class="hy-pager-btn" data-smarthome-action="setDevicesPage" data-smarthome-page="${smarthomeDeviceState.devicesState.page - 1}" ${smarthomeDeviceState.devicesState.page <= 1 ? 'disabled' : ''} aria-label="${escapeHtmlAttr(t('common.prev_page'))}"><i class="fas fa-chevron-left"></i></button>
            <span class="hy-page-index">${smarthomeDeviceState.devicesState.page} / ${totalPages}</span>
            <button type="button" class="hy-pager-btn" data-smarthome-action="setDevicesPage" data-smarthome-page="${smarthomeDeviceState.devicesState.page + 1}" ${smarthomeDeviceState.devicesState.page >= totalPages ? 'disabled' : ''} aria-label="${escapeHtmlAttr(t('common.next_page'))}"><i class="fas fa-chevron-right"></i></button>
        </div>`;
}
