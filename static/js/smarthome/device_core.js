...
export function _isActiveState(state) {
...
function _setDevicesLoading(message = t('integrations.loading_devices')) {
    ...
    if (grid)
        grid.innerHTML = `<div class="hyd-list-placeholder"><i class="fas fa-circle-notch fa-spin"></i>${escapeHtml(message)}</div>`;
    if (pagination)
        pagination.innerHTML = '';
}
...
export function primeDevicesDetailNavigation(opts) {
...
export function _iconClass(spec) {
...
export function _norm(value) {
    return String(value ?? '').trim().toLowerCase();
}
function _entityId(entity) {
    return String(entity?.entity_id || '');
}
export function _entityDomain(entity) {
...
function _syncDevicesStateFromInputs({ resetPage = false } = {}) {
    smarthomeDeviceState.devicesState.query = _norm(document.getElementById('hy-search')?.value || '');
...
function _restoreSmarthomeEntitySnapshot() {
    if (smarthomeDeviceState.integrationEntitiesCache.length || typeof localStorage === 'undefined')
        return false;
    try {
...
function _scheduleSmarthomeLoadRetry(delayMs = 1500) {
    ...
    smarthomeDeviceState.smarthomeLoadRetryTimer = setTimeout(() => {
        ...
        if (!view || view.classList.contains('hidden') || smarthomeDeviceState.integrationEntitiesCache.length)
            return;
...
export async function loadSmarthome(options = {}) {
    ...
    if (smarthomeDeviceState.smarthomeLoadPromise && !force)
    ...
    if (restoredSnapshot)
        ...
        _setDevicesLoading();
    smarthomeDeviceState.smarthomeLoadPromise = (async () => {
        try {
...
function _ensureSmarthomeLiveSubscription() {
    if (smarthomeDeviceState.liveUnsub)
        return;
...
function _applySmarthomeLiveItems(items, isSnapshot) {
    if (!Array.isArray(smarthomeDeviceState.integrationEntitiesCache))
        smarthomeDeviceState.integrationEntitiesCache = [];
    const idx = new Map();
...
export function _syncEntityToggleDom(entity) {
    ...
    const btns = document.querySelectorAll(`button.app-toggle-switch[data-smarthome-entity-id="${CSS.escape(eid)}"], button.app-toggle-switch[data-entity-toggle="${CSS.escape(eid)}"]`);
...
export function _optimisticStateForAction(action, currentState, domain = '') {
...
function _setDevicesViewMode(mode) {
...
function _updateStats() {
    ...
    if (tabs)
        tabs.innerHTML = renderEntityCategoryTabs(smarthomeDeviceState.entityCategoryFilter);
}
...
function _findDeviceGroupByKeyFallback(key) {
    ...
    if (exact)
        return exact;
    if (parsed.deviceId.startsWith('fn:') && groups.length === 1)
        return groups[0];
    return null;
}
function _findDeviceGroupAfterRename(entryId, canonicalDeviceId, entityIds, ieeeAddrs) {
    ...
    if (ieeeAddrs.size) {
        const byIeee = groups.find((d) => d.entry_id === entryId && (d.entities || []).some((e) => {
            const ieee = String(e.attributes?.zigbee_ieee || '').trim();
...
function _findDeviceGroupByEntityId(entityId) {
    return _getAllDeviceGroups().find((d) => (d.entities || []).some((e) => e.entity_id === entityId)) || null;
}
function _syncDeviceGroupEntities(group) {
...
function _getFilteredDeviceGroups() {
    ...
    const filtered = _getAllDeviceGroups().filter((device) => {
        ...
        if (!deviceMatchesCategory(device, smarthomeDeviceState.entityCategoryFilter))
            return false;
        if (sourceFilter !== 'all' && !ents.some((e) => entityMatchesIntegration(e.source || '', sourceFilter)))
            return false;
        if (areaFilter !== 'all') {
        ...
        if (domainFilter === 'active') {
            if (!deviceHasActiveEntity(device))
                return false;
        }
        else if (domainFilter === 'ai') {
            if (!ents.some((e) => e.selected))
                return false;
        }
        else if (domainFilter !== 'all') {
            if (!ents.some((e) => {
                const domain = _entityDomain(e);
                if (domainFilter === 'sensor' && domain === 'binary_sensor')
                    return true;
                return domain === domainFilter;
            }))
                return false;
        }
        if (query && !deviceSearchText(device).includes(query))
            return false;
        return true;
    });
...
export function patchOpenDetailForEntity(entity) {
    return _patchOpenDetailForEntity(entity);
}
function _patchOpenDetailForEntity(entity) {
    const eid = String(entity.entity_id || '');
    if (!eid)
        return false;
    if (smarthomeDeviceState.openEntityId === eid) {
...
function _renderEntityDetailView(entity, options = {}) {
    ...
    if (prevDomain === 'camera' || newDomain === 'camera')
    ...
    wireEntityFriendlyNameEditor(detail, entity, {
        onUpdated: ({ name }) => {
            const idx = smarthomeDeviceState.integrationEntitiesCache.findIndex((e) => e.entity_id === entity.entity_id);
            if (idx >= 0)
                smarthomeDeviceState.integrationEntitiesCache[idx].name = name;
...
function wireDeviceDetailNameEditor(container, device) {
    if (!container || !canRenameIntegrationDevice(device))
        return;
    ...
    const input = root.querySelector('[data-device-name-input]');
    const editBtn = root.querySelector('[data-device-name-edit]');
    const saveBtn = root.querySelector('[data-device-name-save]');
    const cancelBtn = root.querySelector('[data-device-name-cancel]');
    const showView = () => {
    ...
    const showEdit = () => {
        ...
        if (input)
            input.value = String(device.name || device.device_id || '');
        input?.focus();
        input?.select();
    };
    if (editBtn)
    ...
    const submit = async () => {
        ...
        const ieeeAddrs = new Set((device.entities || [])
            .map((e) => String(e.attributes?.zigbee_ieee || '').trim())
            .filter(Boolean));
        try {
...
function _renderDeviceDetailView(device, options = {}) {
    const detail = document.getElementById('hy-entity-detail-view');
    if (!detail)
        return;
...
export function openDeviceDetail(deviceKey, opts = {}) {
...
export function openEntityDetail(entityId, opts = {}) {
    ...
    if (!opts.skipDeviceParent && !smarthomeDeviceState.openDeviceKey) {
        ...
        if (group)
            smarthomeDeviceState.openDeviceKey = group.device_key;
    }
    smarthomeDeviceState.openEntityId = entityId;
    smarthomeModalState.haRowActionsEntityId = entityId;
    renderDeviceCards();
...
export function filterEntityCategory(category) {
...
export function _domainLabel(domain) {
...
export function selectDevicePrimaryEntity(deviceKey, entityId) {
    const key = String(deviceKey || '').trim();
    if (!key)
        return;
    ...
    if (smarthomeDeviceState.openDeviceKey === key) {
        const device = _findDeviceGroupByKey(key);
        if (device)
            _renderDeviceDetailView(_syncDeviceGroupEntities(device));
        return;
    }
    renderDeviceCards();
}
...
function _wireSmarthomeFilterPickerEvents() {
    if (smarthomeDeviceState.filterPickerEventsWired || typeof document === 'undefined')
        return;
    ...
    document.addEventListener('click', (event) => {
        ...
        if (!tgt)
            return;
        if (tgt.closest('[data-smarthome-action="togglePicker"], [data-smarthome-action="selectPickerOption"]'))
            return;
        if (tgt.closest('[data-smarthome-action="selectDevicePrimaryEntity"]'))
            return;
...
export async function copyEntityIdFromRowActions() {
    if (!smarthomeModalState.haRowActionsEntityId)
        return;
    try {
...
export async function toggleAllAI(checked) {
    ...
    try {
        ...
        if (smarthomeDeviceState.openEntityId) {
            const ent = smarthomeDeviceState.integrationEntitiesCache.find((e) => e.entity_id === smarthomeDeviceState.openEntityId);
            if (ent)
                patchEntityDetailDom(ent);
        }
    }
    catch {
        showToast(t('hy.network_error'), 'error');
