...
function _entityId(el) {
    return String(el.dataset.smarthomeEntityId || el.closest('[data-smarthome-entity-id]')?.dataset.smarthomeEntityId || '').trim();
}
...
function _onClick(event) {
    const el = event.target.closest('[data-smarthome-action]');
    if (!el) return;
    _run(el.dataset.smarthomeAction, el, event);
}

function _onChange(event) {
    const el = event.target.closest('[data-smarthome-change]');
    if (!el || !_inSmarthome(el)) return;
    const kind = el.dataset.smarthomeChange;
    const entityId = _entityId(el);
    if (kind === 'toggleSelection') {
        event.stopPropagation();
        _handlers?.toggleSelection?.(entityId, el.checked, event, el);
        return;
    }
    if (kind === 'toggleDerivedSelection') {
        _handlers?.toggleDerivedSelection?.(entityId, el.checked, event, el);
        return;
    }
    if (kind === 'toggleAllAIVisible') {
        _handlers?.toggleAllAIVisible?.(el.checked, event, el);
        return;
    }
    if (kind === 'setDevicesPageSize') {
        _handlers?.setDevicesPageSize?.(el.value, event, el);
        return;
    }
...
function _onInput(event) {
    const el = event.target.closest('[data-smarthome-input]');
    if (!el || !_inSmarthome(el)) return;
