...
import type { EntityStateChangedDetail } from './types/integration.js';
...
async function _fetchSseToken(): Promise<string | null> {
    try {
...
async function _open(slug: string): Promise<void> {
    if (_es) return;
    ...
    _es.addEventListener('message', (e) => {
        try {
            const detail = JSON.parse(String(e.data)) as EntityStateChangedDetail;
    ...
    _es.addEventListener('bridge', (e) => {
        try {
            const detail = JSON.parse(String(e.data)) as Record<string, unknown>;
    ...
    _es.addEventListener('error', () => {
        ...
        _reconnectTimer = setTimeout(() => {
            ...
            if (_refCount > 0 && _currentSlug) void _open(_currentSlug);
        }, 3000);
    });
}

export async function startEntityLiveStream(slug = 'mosquitto'): Promise<void> {
    const normalized = String(slug || 'mosquitto').trim() || 'mosquitto';
    if (!LIVE_STREAM_SOURCES.has(normalized)) return;
    _refCount += 1;
    _currentSlug = normalized;
    _shuttingDown = false;
    if (!_es) await _open(normalized);
}

export function stopEntityLiveStream(): void {
...
window.addEventListener('entity-state-changed', (e: Event) => {
    const detail = (e as CustomEvent<EntityStateChangedDetail>).detail || {};
    ...
    if (Number.isFinite(numericState)) {
        document.querySelectorAll(`input[data-entity-control="${CSS.escape(eid)}"]`).forEach((input) => {
            if (input instanceof HTMLInputElement) input.value = String(numericState);
        });
    }
