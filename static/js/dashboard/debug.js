...
export const DASH_DEBUG_ENABLED = (() => {
    try {
        ...
        if (search.get('dashdebug') === '1') {
            try { localStorage.setItem('hyve_dash_debug', '1'); } catch (_) {}
            return true;
        }
        if (search.get('dashdebug') === '0') {
            try { localStorage.removeItem('hyve_dash_debug'); } catch (_) {}
            return false;
        }
        try { return localStorage.getItem('hyve_dash_debug') === '1'; } catch (_) { return false; }
