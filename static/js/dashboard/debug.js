...
export const DASH_DEBUG_ENABLED = (() => {
    try {
        ...
        if (search.get('dashdebug') === '1') {
            try {
                localStorage.setItem('hyve_dash_debug', '1');
            }
            catch { /* ignore */ }
            return true;
        }
        if (search.get('dashdebug') === '0') {
            try {
                localStorage.removeItem('hyve_dash_debug');
            }
            catch { /* ignore */ }
            return false;
        }
        try {
            return localStorage.getItem('hyve_dash_debug') === '1';
        }
        catch {
            return false;
        }
...
function ensureDashDebugOverlay() {
    if (!DASH_DEBUG_ENABLED || typeof document === 'undefined')
        return null;
    ...
    el.querySelector('[data-dash-dbg-clear]')?.addEventListener('click', () => {
        if (typeof window !== 'undefined')
            window.__hyveDashLog = [];
        renderDashDebugOverlay();
    });
    return el;
}
let dashDebugRenderQueued = false;
function renderDashDebugOverlay() {
    if (!DASH_DEBUG_ENABLED || typeof window === 'undefined')
        return;
    if (dashDebugRenderQueued)
        return;
...
export function dashDebug(tag, info) {
    if (!DASH_DEBUG_ENABLED)
        return;
    try {
        const entry = { t: new Date().toISOString().slice(11, 23), tag: String(tag || ''), info };
        if (typeof window !== 'undefined') {
            if (!Array.isArray(window.__hyveDashLog))
                window.__hyveDashLog = [];
            window.__hyveDashLog.push(entry);
            if (window.__hyveDashLog.length > 200)
                window.__hyveDashLog.shift();
        }
