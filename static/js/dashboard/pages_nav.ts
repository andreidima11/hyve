...
import type { DashboardPagesNavDeps } from '../types/dashboard.js';

let _deps: DashboardPagesNavDeps | null = null;
let _hashRouterBound = false;

function deps(): DashboardPagesNavDeps {
    if (!_deps) throw new Error('Dashboard pages nav not initialized');
    return _deps;
}

export function initDashboardPagesNav(depsIn: DashboardPagesNavDeps): void {
    _deps = depsIn;
}

function persistDashboardPagesNav(pages: Array<Record<string, unknown>>): void {
...
function readDashboardPagesNav(): Array<Record<string, unknown>> {
    try {
...
export function renderDashboardPagesList(): void {
    const d = deps();
...
export function initDashboardSidebarNav(): void {
    const d = deps();
    const cache = d.getDashboardCache();
...
export function resolveCurrentDashboardPageId(): string {
    const d = deps();
    const cache = d.getDashboardCache();
    const pages = Array.isArray(cache.pages) ? cache.pages : [];
    const hasPage = (pageId: string | null | undefined) =>
        !!pageId && (!pages.length || pages.some((page) => String(page?.id || '') === String(pageId)));
    ...
    let storedPage = '';
    try { storedPage = String(localStorage.getItem(DASHBOARD_LAST_PAGE_KEY) || ''); } catch { /* ignore */ }
...
export async function openDashboardPageNav(pageId: string): Promise<void> {
    const d = deps();
    const view = document.getElementById('view-dashboard');
    const onDash = !!view && !view.classList.contains('hidden');
    if (pageId) { try { setHashForPage(String(pageId)); } catch { /* ignore */ } }
    if (!onDash) {
...
export function setHashForPage(pageId: string): void {
    if (!pageId) return;
...
export function readHashPageId(): string | null {
