...
function deps() {
    if (!_deps)
        throw new Error('Dashboard page modal not initialized');
    return _deps;
}
export function initDashboardPageModal(depsIn) {
    _deps = depsIn;
}
function mergeCreatedPageIntoCache(createdPage, newId) {
    const d = deps();
    if (!newId)
        return;
    d.setCurrentPageId(String(newId));
    try {
        localStorage.setItem(DASHBOARD_LAST_PAGE_KEY, String(newId));
    }
    catch (_) { }
    ...
    if (createdPage.title) {
        ...
        try {
            localStorage.setItem('hyve.lastDashboardTitle', cache.title);
        }
        catch (_) { }
    }
    if (createdPage.icon != null)
        cache.icon = String(createdPage.icon || '');
    if (createdPage.columns != null)
        cache.columns = Number(createdPage.columns) || 0;
...
export function openDashboardPageModal(opts = {}) {
    ...
    if (createMode) {
        if (titleEl)
            titleEl.textContent = d.t('dashboard.new_page') || 'New page';
        if (saveBtn)
            saveBtn.textContent = d.t('dashboard.create') || 'Create';
        if (titleInput)
            titleInput.value = '';
        if (iconInput)
            iconInput.value = 'fa-table-cells-large';
        if (columnsInput)
            columnsInput.value = '0';
        if (layoutInput)
            layoutInput.value = DEFAULT_PREFS.layout_mode;
        if (hideInput)
            hideInput.checked = false;
    }
    else {
        if (titleEl)
            titleEl.textContent = d.t('dashboard.edit_page') || 'Edit page';
        if (saveBtn)
            saveBtn.textContent = d.t('common.save') || 'Save';
        if (titleInput)
            titleInput.value = cache.title || DEFAULT_META.title;
        if (iconInput)
            iconInput.value = cache.icon || 'fa-table-cells-large';
        if (columnsInput)
            columnsInput.value = String(cache.columns || 0);
        if (layoutInput)
            layoutInput.value = cache.preferences?.layout_mode || DEFAULT_PREFS.layout_mode;
        if (hideInput)
            hideInput.checked = !(cache.preferences?.show_unavailable ?? DEFAULT_PREFS.show_unavailable);
    }
    ...
    if (defaultInput) {
        ...
        if (row)
            row.classList.toggle('hidden', createMode);
...
export async function deleteDashboardPage() {
    ...
    if (!(await showConfirm(d.t('dashboard.delete_page_confirm') || 'Delete the current page? Its cards and panels will be removed.')))
        return;
    try {
