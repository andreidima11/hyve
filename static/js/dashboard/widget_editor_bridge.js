...
function deps() {
    if (!_deps)
        throw new Error('Dashboard widget editor bridge not initialized');
    return _deps;
}
...
export async function ensureHyveviewEntitySeed() {
    ...
    try {
        ...
        if (mod && typeof mod.seedEntities === 'function')
            mod.seedEntities(seed);
    }
    catch { /* offline ok */ }
}
...
function editorResultToWidgetBody(result, { existingWidget = null } = {}) {
    ...
    if (type === 'climate') {
        const records = enrichEntityRecords((Array.isArray(cfg.entities) ? cfg.entities : []).map(e => typeof e === 'string'
            ? { entity_id: e }
    ...
    if (type === 'camera') {
        ...
        if (records[0]?.title && !Object.prototype.hasOwnProperty.call(cfg, 'title'))
            body.title = records[0].title;
    }
    if (type === 'picture') {
        ...
        if (firstEntity)
            body.entity_id = firstEntity.value;
        else if (!body.entity_id)
            body.entity_id = `picture.gallery_${Date.now()}`;
    }
    if (type === 'fusion_solar') {
        const powerRecords = (Array.isArray(cfg.power_entities) ? cfg.power_entities : [])
            .map((e) => typeof e === 'string'
        ...
        if (!body.source || body.source === 'zigbee2mqtt') {
            ...
            if (ent?.source)
                body.source = ent.source;
        }
    }
    if (result.visibility)
        body.visibility = result.visibility;
