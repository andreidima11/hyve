...
import type {
    DashboardWidget,
    DashboardWidgetLocateResult,
    DashboardWidgetStoreDeps,
} from '../types/dashboard.js';

let _deps: DashboardWidgetStoreDeps | null = null;
...
let _pendingReorder: PendingReorder | null = null;

function deps(): DashboardWidgetStoreDeps {
    if (!_deps) throw new Error('Dashboard widget store not initialized');
    return _deps;
}

export function initDashboardWidgetStore(depsIn: DashboardWidgetStoreDeps): void {
    _deps = depsIn;
}

/** Locate a widget list entry across panels, pages, and top-level widgets. */
export function locateDashboardWidget(widgetId: string): DashboardWidgetLocateResult | null {
    ...
    for (let pageIndex = 0; pageIndex < pages.length; pageIndex++) {
        ...
        const pageWidgets = Array.isArray(page?.widgets) ? (page.widgets as DashboardWidget[]) : null;
        if (pageWidgets) {
        ...
        for (let panelIndex = 0; panelIndex < pagePanels.length; panelIndex++) {
            ...
            const list = Array.isArray(panel?.widgets) ? panel.widgets : null;
            if (!list) continue;
            const idx = list.findIndex((w: DashboardWidget) => w && w.id === widgetId);
...
export function findWidget(widgetId: string): DashboardWidget | null {
...
export function dashboardWidgetById(widgetId: string): DashboardWidget | null {
    return findWidget(widgetId);
}

export function reorderDashboardWidgets(sourceId: string, targetId: string): boolean {
    const d = deps();
    ...
    if (dst.panel && Array.isArray(dst.panel.pages) && dst.panel.pages.length) {
        ...
        const firstPage = dst.panel.pages[0] as { id?: string } | undefined;
        const fallbackPage = firstPage?.id || null;
        if (targetWidget?.page_id) moved.page_id = targetWidget.page_id;
...
export async function persistDashboardOrder(): Promise<void> {
    const d = deps();
    ...
    if (pending.targetPanelId) {
        const body: Record<string, string> = { target_panel_id: pending.targetPanelId };
        ...
        if (!res.ok) {
            ...
            throw new Error(dashApiError((err as { detail?: unknown }).detail, 'dashboard.rearrange_widget_failed'));
        }
        return;
    }
    ...
    const storedWidgets = Array.isArray(section.widgets) ? (section.widgets as DashboardWidget[]) : [];
