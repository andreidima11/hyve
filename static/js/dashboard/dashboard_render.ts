...
import type { DashboardPanel, DashboardRenderDeps, DashboardWidget } from '../types/dashboard.js';

let _deps: DashboardRenderDeps | null = null;
const _panelActivePage = new Map<string, string>();

function deps(): DashboardRenderDeps {
    if (!_deps) throw new Error('Dashboard render not initialized');
    return _deps;
}

export function initDashboardRender(depsIn: DashboardRenderDeps): void {
    _deps = depsIn;
}
...
function renderPanelSection(panel: DashboardPanel, mobileOrderIndex = 0): string {
    ...
    const pages = Array.isArray(panel.pages) ? panel.pages as Array<{ id?: string; title?: string; icon?: string }> : [];
    ...
    const visibleWidgets = visibleDashboardWidgets(activePageId
        ? widgets.filter((w: DashboardWidget) => String(w.page_id || '') === String(activePageId))
        : widgets);
...
export function selectDashboardPanelPage(panelId: string, pageId: string): void {
