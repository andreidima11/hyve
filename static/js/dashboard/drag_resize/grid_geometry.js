...
import { findWidget, getCache, isStandalonePanel, panelColSpan, renderDashboard, widgetSpan, apiCall, dashApiErr, getCurrentPageId, } from './shared.js';
export function _readGridGeometry(gridEl) {
...
export function _visualColSpanForGrid(colSpan, geom) {
...
export function _visualColStartForGrid(colStart, geom, spanCol = 1) {
...
export function _internalColStartForGrid(visualCol, _geom) {
...
export function _pointerToGridCell(gridEl, geom, clientX, clientY, span, offsetX = 0, offsetY = 0) {
...
export function _dashboardPanelRenderedRowSpan(panelEl, geom, fallbackRows = 1) {
    if (!panelEl || !geom)
...
export function _dashboardPanelRenderedColSpan(_panelEl, _geom, fallbackCols = 2) {
...
export function _dashboardPanelSpan(panel) {
...
export function _dashboardPanelElement(rootGrid, panel) {
    if (!rootGrid || !panel)
        return null;
...
export function _dashboardPanelRect(panel, rootGrid) {
...
export function _dashboardRootLayoutItems() {
    const items = [];
...
export function _dashboardRootItemElement(rootGrid, item) {
    if (!rootGrid || !item)
        return null;
...
export function _dashboardRootItemSpan(item) {
    if (item.kind === 'panel')
        return _dashboardPanelSpan(item.raw);
    return widgetSpan(item.raw);
}
export function _dashboardRootItemRect(item, rootGrid) {
...
export function _findDashboardRootSwapCandidate(movingRect, rootGrid) {
    let best = null;
...
export function _resolveDashboardRootOverlaps(movingRect, rootGrid) {
    const rects = new Map();
    for (const item of _dashboardRootLayoutItems()) {
        if (item.key === movingRect.id)
            continue;
...
export function _normalizeDashboardRootLayout(rootGrid) {
...
export function _applyDashboardRootRect(rect) {
    if (!rect)
        return;
...
export async function _persistDashboardRootRect(rect, pageQS = '') {
    if (!rect)
        return;
    if (rect.itemKind === 'panel') {
...
export async function _persistDashboardPanelLayout(panelId, layout) {
...
export function _resolveDashboardPanelOverlaps(movingRect, rootGrid) {
    const rects = new Map();
    for (const panel of _dashboardRootLayoutPanels()) {
        const id = String(panel.id || '');
...
export function _panelWidgets(panelId) {
    if (!panelId) {
...
export function _rectOverlapArea(a, b) {
...
export function _clampColStartForSpan(colStart, colSpan) {
...
export function _findDashboardSwapCandidate(movingRect, panelWidgets, panelGridEl) {
    let best = null;
 ...
 */
export function _widgetRect(widget, panelGridEl) {
 ...
 */
export function _resolveOverlaps(movingRect, panelWidgets, panelGridEl) {
    const rects = new Map();
 ...
 */
export function _normalizePanelLayout(panelWidgets, panelGridEl) {
