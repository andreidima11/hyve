...
function deps() {
    if (!_deps)
        throw new Error('Dashboard page select not initialized');
    return _deps;
}
...
export async function selectDashboardPage(pageId) {
    ...
    try {
        localStorage.setItem(DASHBOARD_LAST_PAGE_KEY, currentPageId);
    }
    catch (_) { }
    ...
    try {
        ...
        if (eagerTitle) {
            ...
            if (headerTitleEl)
                headerTitleEl.textContent = String(eagerTitle);
            const pageTitleEl = document.getElementById('dashboard-page-title');
            if (pageTitleEl)
                pageTitleEl.textContent = String(eagerTitle);
            try {
                localStorage.setItem('hyve.lastDashboardTitle', String(eagerTitle));
            }
            catch (_) { }
        }
    }
    ...
    try {
        ...
        if (snap) {
            ...
            const cache = d.getCache();
            if (cache.page_id)
                d.setCurrentPageId(cache.page_id);
    ...
    try {
        await d.withDashboardTimeout(d.refreshAvailableEntities({ includeEntities: false }), 8000, t('dashboard.refresh_timeout'));
        if (myToken !== _pageNavToken) {
            clearTimeout(watchdog);
            return;
        }
        const freshFp = dashboardSnapshotFingerprint(d.getCache());
        if (!renderedFromSnapshot || freshFp !== snapFp)
            d.renderDashboard();
        if (grid)
            requestAnimationFrame(() => { grid.style.opacity = '1'; });
    }
