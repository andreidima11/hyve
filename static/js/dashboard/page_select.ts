...
import type { DashboardPageSelectDeps } from '../types/dashboard.js';

let _deps: DashboardPageSelectDeps | null = null;
let _pageNavToken = 0;

function deps(): DashboardPageSelectDeps {
    if (!_deps) throw new Error('Dashboard page select not initialized');
    return _deps;
}

export function initDashboardPageSelect(depsIn: DashboardPageSelectDeps): void {
    _deps = depsIn;
}

/** Invalidate in-flight page switches (tab leave, edit reset). */
export function abortDashboardPageNavigation(): void {
    _pageNavToken += 1;
}

export async function selectDashboardPage(pageId: string): Promise<void> {
    if (!pageId) return;
    const d = deps();
    const myToken = ++_pageNavToken;
    d.setCurrentPageId(String(pageId));
    const currentPageId = String(d.getCurrentPageId() || '');
    ...
    try {
        ...
        if (eagerTitle) {
            ...
            if (headerTitleEl) headerTitleEl.textContent = String(eagerTitle);
            const pageTitleEl = document.getElementById('dashboard-page-title');
            if (pageTitleEl) pageTitleEl.textContent = String(eagerTitle);
            try { localStorage.setItem('hyve.lastDashboardTitle', String(eagerTitle)); } catch (_) {}
        }
    } catch (_) {}
    ...
    let snapFp: string | null = null;
    try {
        const snap = getDashboardPageSnapshot(String(d.getCurrentPageId() || ''));
        if (snap) {
    ...
    try {
        ...
        if (gridHasContent) {
            ...
            d.showToast(t('dashboard.refresh_failed', { message }), 'error');
        } else if (gridNow) {
            const message = e instanceof Error ? e.message : t('dashboard.unknown_error');
            gridNow.innerHTML = `<div class="col-span-full p-6 text-sm rounded-2xl border border-red-500/20 bg-red-500/10 text-red-300">
                ...
                <div class="text-xs opacity-80 mb-2">${escapeHtml(message)}</div>
