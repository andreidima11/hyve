...
import type { CodeMirrorEditor, DashboardYamlEditorDeps } from '../types/dashboard.js';

export function createDashboardYamlEditor(deps: DashboardYamlEditorDeps) {
    const {
        apiCall,
        t,
        showToast,
    ...
    let cdnLoading: Promise<boolean> | null = null;
    ...
    function loadJs(src: string, id: string): Promise<void> {
    ...
    async function ensureCodeMirror(): Promise<boolean> {
    ...
    function setStatus(msg: string, kind: 'info' | 'error' | 'ok' | 'busy' = 'info'): void {
        const el = document.getElementById('dashboard-yaml-status');
        if (!el) return;
        el.textContent = msg || '';
        el.className = 'ml-auto text-[11px] ' + (
    ...
    async function fetchYaml(): Promise<string> {
        ...
        if (!pid) throw new Error('Nu există o pagină activă.');
    ...
    function getValue(): string {
        ...
        const ta = document.getElementById('dashboard-yaml-editor') as HTMLTextAreaElement | null;
        return ta ? ta.value : '';
    }
    ...
    async function openDashboardYamlEditor(): Promise<void> {
        ...
        if (nameEl) nameEl.textContent = '— ' + getActivePageName();
    ...
    function closeDashboardYamlEditor(): void {
        const modal = document.getElementById('dashboard-yaml-modal');
        if (!modal) return;
    ...
    async function reloadDashboardYaml(): Promise<void> {
    ...
    async function saveDashboardYaml(): Promise<void> {
        const pid = getActivePageId();
        if (!pid) { setStatus('Nu există pagină activă', 'error'); return; }
        const text = getValue();
        if (!text || !text.trim()) { setStatus(t('dashboard.yaml_empty'), 'error'); return; }
        ...
        try {
            ...
            const data = await res.json().catch(() => ({})) as { detail?: string; yaml?: string };
            if (!res.ok) throw new Error(data.detail || `HTTP ${res.status}`);
            if (data.yaml) setValue(data.yaml);
            setStatus('Salvat ✓', 'ok');
            showToast(t('dashboard.yaml_saved'), 'success');
            try { await reloadDashboard(); } catch (_) {}
        } catch (e) {
