...
function _resolveDeviceId(ent, attrs, entryId, source) {
    ...
    if (fn)
        return `fn:${entryId}:${fn}`;
    if (source === 'derived')
        return String(ent.entity_id || '').trim();
    return String(ent.entity_id || '').trim();
}
export function groupEntitiesIntoDevices(entities) {
    ...
    for (const ent of entities) {
        ...
        if (!groups.has(key)) {
            ...
            groups.set(key, {
                ...
                image_url: String(ent.image_url || attrs.image_url || ''),
...
const CONTROL_DOMAINS = ['light', 'fan', 'switch', 'input_boolean', 'outlet', 'plug'];
const READONLY_DOMAINS = ['sensor', 'binary_sensor', 'number', 'select', 'button', 'event'];
function _inferPrimaryDomain(device) {
...
export function deviceListCategory(device) {
    const dom = device.primary_domain || _inferPrimaryDomain(device);
    if (dom === 'light')
        return 'light';
    if (dom === 'camera')
        return 'camera';
    if (['climate', 'water_heater', 'fan'].includes(dom))
        return 'climate';
    if (['sensor', 'binary_sensor'].includes(dom))
        return 'sensor';
    if (['switch', 'input_boolean', 'outlet', 'plug'].includes(dom))
        return 'switch';
    const known = new Set([
...
export function sortDeviceEntities(device) {
    const order = {
...
export function canRenameIntegrationDevice(device) {
    const slug = String(device.source_slug || '').trim();
    const deviceId = String(device.device_id || '').trim();
    if (!slug || slug === 'derived' || !deviceId)
        return false;
