...
export async function installAddon(slug) {
    ...
    if (btn) {
        btn.disabled = true;
        btn.innerHTML = `<i class="fas fa-spinner fa-spin"></i> ${escapeHtml(t('hy.addon_installing'))}`;
    }
    try {
...
export async function openAddonConfigModal(slug) {
    ...
    if (titleEl)
        titleEl.textContent = addon.name || slug;
    if (iconEl)
        iconEl.className = `${addon.icon || 'fas fa-puzzle-piece'}`;
    const schema = addon.config_schema || [];
    const cfg = addon.state?.config || {};
    const suggestions = addon.config_suggestions;
    const admin = isAdmin();
    fieldsEl.innerHTML = schema.map(field => renderAddonConfigField(field, resolveAddonConfigValue(field, cfg, suggestions), admin, 'data-addon-key')).join('');
    ...
    if (watchdogSection)
        watchdogSection.classList.toggle('hidden', !addon.start_command);
    ...
    if (addonEntContainer)
        addonEntContainer.innerHTML = '';
...
export async function checkAddonHealth() {
    ...
    if (resultEl) {
        ...
        resultEl.className = 'text-xs rounded-xl p-3 bg-slate-900 border border-white/5 text-slate-400';
        resultEl.textContent = t('common.checking');
    }
    ...
    try {
        ...
        if (data.ok) {
            if (resultEl) {
                resultEl.className = 'text-xs rounded-xl p-3 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400';
                resultEl.textContent = `✓ ${t('hy.addon_health_connected', { detail: data.detail || 'OK' })}`;
            }
        }
        else {
            if (resultEl) {
                resultEl.className = 'text-xs rounded-xl p-3 bg-red-500/10 border border-red-500/20 text-red-400';
                resultEl.textContent = `✗ ${t('hy.addon_health_error', { detail: formatHealthError(data.detail) })}`;
            }
        }
    }
    catch (e) {
        if (resultEl) {
            resultEl.className = 'text-xs rounded-xl p-3 bg-red-500/10 border border-red-500/20 text-red-400';
            resultEl.textContent = `✗ ${t('hy.addon_health_network_error')}`;
        }
    }
    if (btn)
        btn.disabled = false;
