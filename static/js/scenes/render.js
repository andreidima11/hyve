...
export function _iconClass(spec, fallback = 'fas fa-film') {
...
export function _entityDomain(entityId) {
    const idx = String(entityId || '').indexOf('.');
    return idx > 0 ? entityId.slice(0, idx) : '';
}
export function _serviceSelectHtml(idx, currentService) {
    const labels = {
...
export function _entryRowHtml(entry, idx) {
...
export function _readEditorEntriesFromDOM() {
...
export function _renderEntityPickerList(query) {
