...
function _queueNotificationSettingsAutoSave() {
    if (notifState.settingsHydrating) return;
    if (notifState.autoSaveTimer) clearTimeout(notifState.autoSaveTimer);
...
function _bindNotificationSettingsAutoSave() {
    if (notifState.autoSaveBound) return;
...
function _stopNotifWsStatusPolling() {
    if (notifState.wsStatusTimer) { clearInterval(notifState.wsStatusTimer); notifState.wsStatusTimer = null; }
}
