...
export function lightHasColor(attrs, caps) {
    if (caps.color)
        return true;
...
export function lightHasColorTemp(attrs, caps) {
    if (caps.color_temp)
        return true;
...
export function lightBrightnessScale(caps) {
...
export function renderHyColorPickerMarkup(hex, hiddenAttrs, esc, escAttr, labels, options) {
    const { h, s, v } = hexToHsv(hex);
    const safeHex = escAttr(/^#[0-9a-f]{6}$/i.test(hex) ? hex.toLowerCase() : '#ffffff');
    const compact = options?.compact ?? false;
    const presets = COLOR_PRESETS.map((preset) => (`<button type="button" class="hy-color-picker__preset" data-hy-color-preset="${escAttr(preset)}"
            style="--hy-preset-color:${escAttr(preset)}" title="${escAttr(preset)}" aria-label="${escAttr(preset)}"
            data-entity-stop="1"></button>`)).join('');
    return `
...
function _setPickerHsv(picker, h, s, v) {
...
function _syncColorPicker(picker, dispatchChange) {
    const { h, s, v } = _pickerHsv(picker);
    const hex = hsvToHex(h, s, v);
    const preview = picker.querySelector('[data-hy-color-preview]');
    const hexLabel = picker.querySelector('[data-hy-color-hex]');
    const cursor = picker.querySelector('[data-hy-color-cursor]');
    const hidden = picker.querySelector('[data-hy-color-value]');
    const hueInput = picker.querySelector('[data-hy-color-hue]');
    const sb = picker.querySelector('[data-hy-color-sb]');
    if (preview)
    ...
    if (hueInput && Number(hueInput.value) !== h)
        hueInput.value = String(h);
    if (sb)
        sb.setAttribute('aria-valuenow', String(s));
    if (hidden) {
        hidden.value = hex;
        if (dispatchChange) {
...
function _sbFromPointer(sb, clientX, clientY) {
    const rect = sb.getBoundingClientRect();
    const x = Math.max(0, Math.min(1, (clientX - rect.left) / rect.width));
    const y = Math.max(0, Math.min(1, (clientY - rect.top) / rect.height));
    return { s: Math.round(x * 100), v: Math.round((1 - y) * 100) };
}
function _applySbPointer(sb, clientX, clientY, dispatchChange) {
...
export function initHyColorPickerBindings() {
    if (_colorPickerBound)
        return;
    _colorPickerBound = true;
    document.addEventListener('input', (event) => {
        const target = event.target;
        if (!(target instanceof HTMLInputElement) || !target.matches('[data-hy-color-hue]'))
            return;
    ...
    document.addEventListener('pointerdown', (event) => {
        const sb = event.target.closest('[data-hy-color-sb]');
        if (!(sb instanceof HTMLElement))
            return;
        _dragSb = sb;
    ...
    const endSbDrag = (event) => {
        if (!_dragSb)
            return;
        try {
            _dragSb.releasePointerCapture(event.pointerId);
        }
        catch { /* ignore */ }
    ...
    document.addEventListener('click', (event) => {
        const preset = event.target.closest('[data-hy-color-preset]');
        if (!(preset instanceof HTMLButtonElement))
            return;
...
export function renderLightControlsMarkup(entity, slug, ctrlAttrs, esc, escAttr, labels, options) {
    ...
    if (!flags.hasBrightness && !flags.hasColor && !flags.hasColorTemp)
        return '';
    const compact = options?.compact ?? false;
    ...
    if (flags.hasColor) {
        ...
        <div class="${sectionClass}" data-int-light-controls="1">
            ${renderHyColorPickerMarkup(flags.colorHex, `${ctrlAttrs(slug, eid, 'set', null, { stop: true })} data-int-input="color" data-entity-stop="1"`, esc, escAttr, { color: labels.color, hue: labels.hue }, { compact })}
        </div>`);
    }
...
export function resolveLightControlFlags(entity, isOn = String(entity.state || '').toLowerCase() === 'on') {
