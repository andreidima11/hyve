...
export const MOMENTARY_DOMAINS = ['button', 'script', 'scene'];
export function isMomentaryDomain(domain) {
    return MOMENTARY_DOMAINS.includes(String(domain || '').toLowerCase());
}
/** State text for entity rows/cards — avoids showing ``unknown`` for empty button states. */
export function entityStateForDisplay(domain, state, tState) {
    if (isMomentaryDomain(domain))
...
export function selectOptionsFromEntity(attrs, caps) {
...
export function renderSelectControlHtml(slug, eid, attrs, caps, currentState, ctrlAttrs, escapeHtmlAttr, escapeHtml) {
