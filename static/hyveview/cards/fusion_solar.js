...
import { renderFlowCard, updateFlowCard } from './power_flow_card.js';
...
function _fmtEnergy(n) {
  if (n == null || !Number.isFinite(n)) return '—';
  if (Math.abs(n) >= 1000) return `${(n / 1000).toFixed(1)} MWh`;
  return `${_fmtNum(n, n >= 10 ? 0 : 1)} kWh`;
}

function _fmtPowerKw(n) {
  if (n == null || !Number.isFinite(n)) return '—';
  return `${_fmtNum(n, Math.abs(n) >= 10 ? 0 : 1)} kW`;
}

function _fmtPowerSmart(kw) {
  if (kw == null || !Number.isFinite(kw)) return '—';
  if (Math.abs(kw) < 1) return `${Math.round(Math.abs(kw) * 1000)} W`;
  const n = Math.abs(kw);
...
export class HyveviewFusionSolarCard extends HyveviewCardBase {
  ...
  setState(entity) {
    ...
    if (role === 'power') {
      ...
      this._config.current_state = entity.state;
      if (entity.unit) this._config.unit = entity.unit;
      this._config.available = entity.available !== false;
    } else {
  ...
  _isCompact() {
    const span = this._config?._span || { row: 2, col: 2 };
    return span.row <= 1;
  }
  ...
  _batteryState() {
    ...
    return {
      power: this._stateValKw('battery'),
      soc: this._stateVal('battery_soc'),
    };
  }
      ...
      this.innerHTML = `
        <div class="hv-fsolar hv-fsolar--compact">
          ...
          <div class="hv-fsolar__metrics hv-fsolar__metrics--live">
            ${this._metricHtml('live', METRICS.live.slice(0, 3), escape)}
          </div>
        </div>`;
      return;
    }
  ...
  _updateFlow(m) {
    ...
    if (!root) return;
    updateFlowCard(root, m, { fmtPowerSmart: _fmtPowerSmart }, this._batteryState());
  }
  ...
  _applyState() {
    ...
    if (statusEl) {
      if (m.producing && m.exporting) statusEl.textContent = 'Injectezi';
      else if (m.producing) statusEl.textContent = 'Producție';
      else if ((m.load ?? 0) > 0.05) statusEl.textContent = 'Consum';
      else statusEl.textContent = 'Inactiv';
      statusEl.dataset.mode = m.producing ? (m.exporting ? 'export' : 'solar') : 'idle';
    }

    if (this._isCompact()) {
      for (const spec of METRICS.live) {
        this._setMetric(spec.key, spec.fmt(m[spec.key]));
      }
    } else {
      this._updateFlow(m);
    }
