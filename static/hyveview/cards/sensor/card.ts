...
function _parseNumeric(rawState: unknown) {
...
function _entityDomain(entityId: unknown) {
  return String(entityId || '').split('.')[0].trim().toLowerCase();
}

/** Sparklines only for numeric `sensor.*` entities — not binary_sensor / text states. */
function _supportsSparkline(widget: CardWidget | null | undefined) {
  return _entityDomain(widget?.entity_id) === 'sensor';
}

function _dashboardRows(widget: CardWidget | null | undefined, article: HTMLElement | null | undefined) {
  ...
  const fromWidget = parseInt(String(widget?.row_span ?? ''), 10);
...
function _isCompactLayout(widget: CardWidget | null | undefined, numeric: number | null, article: HTMLElement | null | undefined) {
...
function _displayLabel(widget: CardWidget | null | undefined) {
  const w = widget || {};
  const eid = String(w.entity_id || '');
  const wAttrs = (w.attributes && typeof w.attributes === 'object' ? w.attributes : {}) as Record<string, unknown>;
  const friendly = String(wAttrs.friendly_name || w.entity_name || '').trim();
...
export class HyveviewSensorCard extends HyveviewCardBase {
  ...
  protected _sparkSlot: HTMLElement | null;
  protected _trendEl: HTMLElement | null;
  ...
  _syncCompactClass(compact: boolean) {
  ...
  _render() {
    ...
    const iconSpec = (typeof host.widgetIcon === 'function' ? host.widgetIcon(w) : w.icon)
      || host.entityIcon(String(w.domain || ''));
  ...
  _applyValue(rawState: unknown, presetUnit: unknown, entityId?: string) {
    const w = (this._config || {}) as CardWidget;
    const { numeric, valueDisplay, tail } = _parseNumeric(rawState);
    const unit = String(presetUnit || tail || '');
    ...
    if (!compact && numeric != null && entityId) {
      ...
      if (prev) {
        ...
        if (Number.isFinite(prevVal)) {
          if (numeric > prevVal) trendDir = 'up';
          else if (numeric < prevVal) trendDir = 'down';
        }
      }
