...
function _parseNumeric(rawState) {
    ...
    if (!m)
        return { numeric: null, valueDisplay: String(rawState || ''), tail: '' };
...
function _isCompactLayout(widget, numeric, article) {
    if (_entityDomain(widget?.entity_id) === 'binary_sensor')
        return true;
    if (_dashboardRows(widget, article) <= 1)
        return true;
    if (numeric == null)
        return true;
    return false;
...
function _displayLabel(widget) {
    ...
    const wAttrs = (w.attributes && typeof w.attributes === 'object' ? w.attributes : {});
    const friendly = String(wAttrs.friendly_name || w.entity_name || '').trim();
    const label = widgetTitle(w);
    if ((!label || label === eid) && friendly)
        return friendly;
...
export class HyveviewSensorCard extends HyveviewCardBase {
    ...
    _syncCompactClass(compact) {
        ...
        if (article)
            article.classList.toggle('hyve-dashboard-card--sensor-compact', this._compact);
    }
    _render() {
        const w = (this._config || {});
    ...
    _applyValue(rawState, presetUnit, entityId) {
        ...
        if (this._sparkSlot) {
            ...
            if (!showSpark)
                this._sparkSlot.innerHTML = '';
        }
    }
    _maybeEnhanceSparkline() {
        if (!this._sparkSlot || this._sparkSlot.hidden)
            return;
        try {
            host.enhanceSparklinesIn(this);
        }
        catch (_) { }
    }
}
HyveviewSensorCard.meta = {
    name: 'Sensor',
    description: 'Live numeric/text value with trend arrow and optional sparkline.',
    icon: '📊',
};
HyveviewSensorCard.schema = {
    fields: [
