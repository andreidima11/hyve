...
export class HyveviewPictureCard extends HyveviewCardBase {
    static getStubConfig(_entityId) {
        return {
            title: '',
    ...
    setConfig(config) {
        if (!config)
            throw new Error('Picture card requires config');
        ...
        let sources = Array.isArray(cfg.sources)
            ? cfg.sources.filter(s => s && s.value)
            : [];
    ...
    _bindSwipe(el) {
        ...
        el.addEventListener('mouseleave', () => { if (this._isDragging)
            onEnd(); });
    }
    ...
    _loadAllImages() {
        this._sources.forEach((src, i) => {
            ...
            this._resolveUrl(src).then((url) => {
                if (url && this._slides[i])
                    this._slides[i].src = url;
            }).catch(() => { });
        });
    }
    ...
    _startRefresh() {
        ...
        const nested = cfg.config && typeof cfg.config === 'object' ? cfg.config : {};
