...
export class HyveviewPictureCard extends HyveviewCardBase {
  ...
  protected _dots: HTMLButtonElement[] = [];
  protected _frame: HTMLElement | null = null;
  protected _isDragging = false;
  protected _observer: IntersectionObserver | null = null;
  protected _onVisibility: () => void;
  protected _refreshTimer: ReturnType<typeof setInterval> | null = null;
  protected _slides: HTMLImageElement[] = [];
  protected _sources: PictureSource[] = [];
  protected _touchDelta = 0;
  protected _touchStartX = 0;
  protected _track: HTMLElement | null = null;
  ...
  setConfig(config: CardWidget | null | undefined) {
    if (!config) throw new Error('Picture card requires config');
    this._config = { interval: DEFAULT_INTERVAL, ...config };
    const cfg = (config.config && typeof config.config === 'object' ? config.config : config) as Record<string, unknown>;
    let sources = Array.isArray(cfg.sources)
      ? (cfg.sources as PictureSource[]).filter(s => s && s.value)
      : [];
    if (!sources.length && Array.isArray(config.sources)) {
      sources = (config.sources as PictureSource[]).filter(s => s && s.value);
    }
    if (!sources.length) {
      const eid = String(config.entity || config.entity_id || '');
  ...
  _addOverlay(container: HTMLElement, title: string) {
    if (!title) return;
    const overlay = document.createElement('div');
    overlay.className = 'hyve-dashboard-card__camera-overlay';
    const esc: Record<string, string> = { '<': '&lt;', '>': '&gt;', '&': '&amp;', '"': '&quot;' };
    overlay.innerHTML = `<span class="hyve-dashboard-card__camera-title">${title.replace(/[<>&"]/g, c => esc[c] || c)}</span>`;
    container.appendChild(overlay);
  }

  _renderDots(container: HTMLElement) {
  ...
  _bindSwipe(el: HTMLElement) {
    let startX = 0, startY = 0, moved = false;
    const track = () => this._track;
    const onStart = (x: number, y: number) => {
      const t = track();
      if (!t) return;
      startX = x; startY = y; this._isDragging = true; moved = false; t.style.transition = 'none';
    };
    const onMove = (x: number, y: number) => {
      const t = track();
      if (!this._isDragging || !t) return;
      const dx = x - startX;
      const dy = y - startY;
    ...
    const onEnd = () => {
      const t = track();
      if ((!this._isDragging && !moved) || !t) return;
  ...
  _goTo(idx: number, animate = true) {
    const track = this._track;
    if (!track) return;
    this._currentIdx = Math.max(0, Math.min(idx, this._sources.length - 1));
    if (!animate) track.style.transition = 'none';
    track.style.transform = `translateX(-${this._currentIdx * 100}%)`;
    if (!animate) requestAnimationFrame(() => { track.style.transition = 'transform 0.3s ease'; });
  ...
  _resolveUrl(source: PictureSource) {
  ...
  _startRefresh() {
    ...
    const nested = cfg.config && typeof cfg.config === 'object' ? cfg.config as Record<string, unknown> : {};
    const cfgInterval = nested.interval ?? cfg.interval;
