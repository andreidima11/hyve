...
export class HyveviewTileCard extends HyveviewCardBase {
    ...
    _customIcon(w = (this._config || {})) {
        if (typeof host.widgetIcon === 'function')
            return host.widgetIcon(w);
    ...
    _applyState(rawState, presetUnit, available) {
        ...
        if (article) {
            ...
            if (w.entity_id)
                article.setAttribute('data-entity-id', w.entity_id);
        }
        if (this._toggleEl)
            this._toggleEl.setAttribute('data-on', on ? 'true' : 'false');
        if (this._unavailableBadge)
            this._unavailableBadge.hidden = available !== false;
    }
}
HyveviewTileCard.meta = {
    name: 'Tile',
    description: 'Generic clickable tile (button / switch / scene / info).',
    icon: '🔘',
};
HyveviewTileCard.schema = {
    fields: [
