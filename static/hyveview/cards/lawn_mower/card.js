...
export class HyveviewLawnMowerCard extends HyveviewCardBase {
    ...
    _applyState() {
        ...
        if (this._startBtn)
            this._startBtn.setAttribute('data-active', meta.on ? 'true' : 'false');
        if (this._stopBtn)
            this._stopBtn.setAttribute('data-active', (state === 'idle' || state === 'paused') ? 'true' : 'false');
