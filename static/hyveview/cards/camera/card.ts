...
export class HyveviewCameraCard extends HyveviewCardBase {
  protected _entityState: import('../../types/card.js').HyveviewEntityState | null;
  protected _img: HTMLImageElement | null;
  protected _loader: HTMLElement | null;
  protected _muteBtn: HTMLButtonElement | null;
  protected _observer: IntersectionObserver | null;
  protected _onVisibility: () => void;
  protected _refreshTimer: ReturnType<typeof setInterval> | null;
  ...
  static schema = {
    fields: [
      ...
      { key: 'autoplay', label: t('dashboard.camera.autoplay'), type: 'boolean', default: true, inline: true, hint: t('dashboard.camera.autoplay_hint') },
  ...
  _render() {
    ...
    const entityId = String(cfg.entity || cfg.entity_id || '');
    ...
    this._loader = loaderWrap.firstElementChild as HTMLElement | null;
    if (this._loader) body.appendChild(this._loader);
  ...
  _snapshotUrl() {
    return cameraMediaUrl(this._entityId(), 'snapshot');
  }

  _streamUrl() {
    return cameraMediaUrl(this._entityId(), 'stream');
  }

  _playUrl() {
    return cameraMediaUrl(this._entityId(), 'play');
  }

  _ensureMuteButton(body: HTMLElement) {
  ...
  _startWebmLive() {
    ...
    this._ensureMuteButton(body as HTMLElement);
    if (!this._video) {
  ...
  _startRefresh() {
    ...
    const interval = Math.max(2, Number(cfg.interval) || DEFAULT_INTERVAL) * 1000;
  ...
  _setLoading(on: boolean, isError = false) {
