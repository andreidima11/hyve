...
export class HyveviewCameraCard extends HyveviewCardBase {
    ...
    setConfig(config) {
        ...
        if (!config || (!config.entity && !ents.length))
            throw new Error('Camera card requires at least one entity');
        this._config = {
            mode: 'snapshot',
            interval: DEFAULT_INTERVAL,
            ...config,
    ...
    _startWebmLive() {
        ...
        if (this._img)
            this._img.dataset.mode = 'live-webm';
    }
    _fallbackMjpegLive() {
        this._startMjpegLive();
    }
    ...
    _startRefresh() {
        ...
        this._refreshTimer = setInterval(() => {
            if (this._img && this._img.dataset.mode === 'snapshot') {
                this._snapshotUrl().then((url) => { if (this._img)
                    this._img.src = url; }).catch(() => { });
            }
        }, interval);
    }
    _setLoading(on, isError = false) {
        ...
        if (isError === true)
            showCameraLoaderError(this._loader);
        else
