...
export class HyveviewWeatherRichCard extends HyveviewCardBase {
    ...
    _render() {
        ...
        const spanRaw = (w._span && typeof w._span === 'object' ? w._span : { row: 1, col: 1 });
    ...
    _tickClock() {
        const now = new Date();
        if (this._cpDateEl)
            this._cpDateEl.textContent = _formatLongDate(now);
        if (this._cpTimeEl)
            this._cpTimeEl.textContent = _formatTime(now);
    }
    _applyState() {
        const w = (this._config || {});
        ...
        if (this._isCompact) {
            if (this._tempEl)
                this._tempEl.textContent = tempStr;
            if (this._iconEl)
                this._iconEl.className = 'hyve-dashboard-card__weather-icon ' + iconCls;
        }
        else {
            const titleFull = widgetTitle(w);
            if (this._cpCityEl)
                this._cpCityEl.textContent = titleFull;
            if (this._cpTempEl)
                this._cpTempEl.textContent = tempRaw != null ? `${Math.round(Number(tempRaw))}°` : '—';
            if (this._cpDescEl)
                this._cpDescEl.textContent = cond ? cond.charAt(0).toUpperCase() + cond.slice(1) : '';
            if (this._cpIconEl)
                this._cpIconEl.className = 'hv-cp-weather__icon ' + iconCls;
            if (this._cpHumidityEl)
                this._cpHumidityEl.textContent = attrs.humidity != null ? String(Math.round(attrs.humidity)) : '—';
            if (this._cpWindEl) {
            ...
            if (this._cpHighEl)
                this._cpHighEl.textContent = hi != null ? String(Math.round(hi)) : '—';
            if (this._cpLowEl)
                this._cpLowEl.textContent = lo != null ? String(Math.round(Number(lo))) : '—';
                ...
                tiles.forEach((tile, idx) => {
                    ...
                    if (!day) {
                        ...
                        if (tempEl)
                            tempEl.textContent = '—';
                        if (iconEl)
                            iconEl.className = 'hv-cp-weather__day-icon fas fa-cloud';
                        if (hiEl)
                            hiEl.textContent = '—';
                        if (loEl)
                            loEl.textContent = '—';
                    ...
                    if (nameEl)
                        nameEl.textContent = String(dayLabel);
                    if (tempEl)
                        tempEl.textContent = dayTempVal != null ? String(Math.round(dayTempVal)) : '—';
                    if (iconEl)
                        iconEl.className = 'hv-cp-weather__day-icon ' + dayIcon;
                    if (hiEl)
                        hiEl.textContent = dayHi != null ? String(Math.round(dayHi)) : '—';
                    if (loEl)
                        loEl.textContent = dayLo != null ? String(Math.round(dayLo)) : '—';
                });
            }
        }
