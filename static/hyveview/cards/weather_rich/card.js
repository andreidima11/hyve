...
export class HyveviewWeatherRichCard extends HyveviewCardBase {
  ...
  _render() {
    ...
    const span = w._span || { row: 1, col: 1 };
  ...
  _applyState() {
    ...
    const tempStr = tempRaw != null ? `${Math.round(tempRaw)}°` : '—';
    ...
    if (this._isCompact) {
      ...
      if (this._cpTempEl) this._cpTempEl.textContent = tempRaw != null ? `${Math.round(tempRaw)}°` : '—';
      ...
      if (this._cpHumidityEl) this._cpHumidityEl.textContent = attrs.humidity != null ? Math.round(attrs.humidity) : '—';
      if (this._cpWindEl) {
        this._cpWindEl.textContent = attrs.wind_speed != null
          ? `${Math.round(attrs.wind_speed)} ${attrs.wind_speed_unit || 'km/h'}`
          : '—';
      }
      ...
      if (this._cpHighEl) this._cpHighEl.textContent = hi != null ? Math.round(hi) : '—';
      if (this._cpLowEl)  this._cpLowEl.textContent  = lo != null ? Math.round(lo) : '—';
      ...
      if (this._cpForecastEl) {
        ...
        tiles.forEach((tile, idx) => {
          ...
          const dayLabel = dt && !isNaN(dt) ? DAY_NAMES[dt.getDay()] : (day.day || '·');
          ...
          if (tempEl) tempEl.textContent = dayTempVal != null ? Math.round(dayTempVal) : '—';
          if (iconEl) iconEl.className = 'hv-cp-weather__day-icon ' + dayIcon;
          if (hiEl) hiEl.textContent = dayHi != null ? Math.round(dayHi) : '—';
          if (loEl) loEl.textContent = dayLo != null ? Math.round(dayLo) : '—';
        });
      }
    }
