...
export function renderSchemaForm(container, schema, initialValues = {}) {
    ...
    for (const f of fields) {
        ...
        if (f.type === 'select') {
            ...
            if (f.inline)
                wrap.classList.add('hv-field--inline');
            ...
            const _renderRows = () => {
                ...
                _rows.forEach((row, idx) => {
                    ...
                    if (row.entity_id && !matches.some((e) => e.entity_id === row.entity_id)) {
                        ...
                        o.textContent = `${row.entity_id} (offline)`;
                        sel.appendChild(o);
                    }
            ...
            if (_rows.length === 0)
                _rows.push({ entity_id: '', unique_id: '', title: '', subtitle: '' });
            _renderRows();
