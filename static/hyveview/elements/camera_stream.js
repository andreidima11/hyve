...
async function _snapshotUrl(entity) {
    return cameraMediaUrl(entity, 'snapshot');
}
async function _streamUrl(entity) {
    return cameraMediaUrl(entity, 'stream');
}
async function _playUrl(entity) {
...
class HyveCameraStream extends HTMLElement {
    ...
    attributeChangedCallback(name, _old, value) {
        ...
        if (this._img)
            this._reevaluate(/* forceReset */ true);
    }
    // Public API ------------------------------------------------------------
    ...
    _startGo2rtcLive() {
        ...
        this._mseRequested = false;
        const flush = () => {
            if (!this._mseBuffer || this._mseBuffer.updating || !this._mseQueue.length)
                return;
            try {
                const chunk = this._mseQueue.shift();
                if (chunk)
                    this._mseBuffer.appendBuffer(chunk);
            }
            catch (_) {
    ...
    _startWebmLive() {
        ...
        _playUrl(this._entity).then((url) => {
            if (this._video && this.isConnected)
                this._video.src = url;
    ...
    _startMjpegLive() {
        ...
        if (this._img.dataset.streamMode === 'live' && this._img.dataset.entityId === this._entity)
            return;
    ...
    _startRefresh() {
        ...
        if (wantLive) {
            ...
            if (sameEntity && (mode === 'live' || mode === 'live-webm' || mode === 'live-go2rtc'))
                return;
            if (this._go2rtcLive)
                this._startGo2rtcLive();
            else if (this._webmLive)
                this._startWebmLive();
            else
                this._startMjpegLive();
            return;
        ...
        this._refreshTimer = setInterval(() => {
            if (!this.isConnected || document.hidden || this._paused)
                return;
            if (!this._visible && !this._buffered && !this._forceActive)
                return;
            _snapshotUrl(this._entity).then((url) => {
                if (this._img && this.isConnected)
                    this._img.src = url;
