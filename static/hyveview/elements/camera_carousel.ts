...
type CarouselStream = (HyveCameraStream | HTMLElement) & {
  pauseStream?: () => void;
  resumeStream?: () => void;
};
...
interface CameraCapabilities {
  ...
  speaker_volume_mutable?: boolean;
  microphone_mutable?: boolean;
  supports_talk?: boolean;
  talk_methods?: string[];
  two_way_audio_capable?: boolean;
  go2rtc_available?: boolean;
  has_audio?: boolean;
  source?: string;
}

function _parseEntities(raw: string): CarouselEntity[] {
  if (!raw) return [];
  try {
...
class HyveCameraCarousel extends HTMLElement {
  ...
  private _index = 0;
  private _stream: CarouselStream | null = null;
  private _streams = new Map<string, CarouselStream>();
  private _caps: CameraCapabilities | null = null;
  private _capsEntity = '';
  private _menuOpen = false;
  private _speakerMuted = true;
  private _micMuted = true;
  private _speakerVolume = 50;
  private _pttActive = false;
  private _pttRecorder: MediaRecorder | null = null;
  private _pttChunks: Blob[] = [];
  private _touchStartX: number | null = null;
  private _menuListenersBound = false;
  private _audioDefaultsApplied = false;
  private _built = false;
  private _hostVisible = true;
  private _hostObserver: IntersectionObserver | null = null;
  ...
  private _volRow: HTMLElement | null = null;
  private _micRow: HTMLElement | null = null;
  private _pttBtn: HTMLButtonElement | null = null;
  private _menuHint: HTMLElement | null = null;
  private _volSlider: HTMLInputElement | null = null;
  private _spkBtn: HTMLButtonElement | null = null;
  private _micBtn: HTMLButtonElement | null = null;
  private _viewport: HTMLElement | null = null;
  private _onMenuReposition: (() => void) | null = null;
  private _onDocClick: ((ev: MouseEvent) => void) | null = null;
    ...
    if ('IntersectionObserver' in window) {
      this._hostObserver = new IntersectionObserver((entries) => {
        for (const e of entries) {
          ...
          if (visible === this._hostVisible) continue;
          this._hostVisible = visible;
          if (visible) this.resumeStream();
          else this.pauseStream();
        }
      }, { threshold: 0.01 });
  ...
  disconnectedCallback() {
    this._stopPtt();
    this.pauseStream();
    if (this._hostObserver) { this._hostObserver.disconnect(); this._hostObserver = null; }
    this._teardownMenuListeners();
    this._closeMenu();
  }
  ...
  _applyAudioDefaultsFromAttrs({ force = false } = {}) {
    if (this._audioDefaultsApplied && !force) return;
  ...
  _clearAllStreams() {
    ...
    for (const entityId of [...this._streams.keys()]) this._removeStream(entityId);
    this._stream = null;
  }

  pauseStream() {
    this._ensureStreamMap();
    this._streams.forEach((stream) => { try { stream.pauseStream?.(); } catch (_) {} });
  }

  resumeStream() {
    this._ensureStreamMap();
    if (!this._stream && this._entities.length) this._syncPreloadStreams();
    this._streams.forEach((stream) => { try { stream.resumeStream?.(); } catch (_) {} });
  }
  ...
  _preloadIndices(): Set<number> {
    ...
    if (this._preloadScope === 'all') {
      this._entities.forEach((_, i) => { if (i !== this._index) indices.add(i); });
      return indices;
    }
    const n = this._entities.length;
    const prev = (this._index - 1 + n) % n;
    const next = (this._index + 1) % n;
    if (prev !== this._index) indices.add(prev);
    if (next !== this._index && next !== prev) indices.add(next);
    return indices;
  }

  _removeStream(entityId: string): void {
    this._ensureStreamMap();
    const stream = this._streams.get(entityId);
    if (!stream) return;
    try { stream.pauseStream?.(); } catch (_) {}
    stream.remove();
    this._streams.delete(entityId);
    if (this._stream === stream) this._stream = null;
  }

  _entityStreamConfig(ent: CarouselEntity): { webm: boolean; go2rtc: boolean; agora: boolean } {
    const cache = (window as Window & { _dashboardCache?: { available_entities?: Array<{ entity_id?: string; attributes?: Record<string, unknown> }> } })._dashboardCache;
    ...
    if (webm === false) webm = cameraPreferWebmPlayer(attrs);
  ...
  _streamUsesAgora(stream: CarouselStream | null | undefined): boolean {
    return String(stream?.tagName || '').toLowerCase() === 'hv-mammotion-camera';
  }

  _setStreamAttr(stream: CarouselStream, name: string, value: string): void {
    if (stream.getAttribute(name) !== value) stream.setAttribute(name, value);
  }

  _applyStreamAttrs(stream: CarouselStream, ent: CarouselEntity, { active = false, buffered = false } = {}): void {
  ...
  _createStreamElement(ent: CarouselEntity): CarouselStream {
    if (this._entityStreamConfig(ent).agora) {
      const stream = document.createElement('hv-mammotion-camera') as CarouselStream;
      stream.className = 'hyve-dashboard-card__camera-player hv-camera-carousel__stream hv-camera-carousel__stream--agora';
      return stream;
    }
    const stream = document.createElement('hv-camera-stream') as CarouselStream;
    stream.className = 'hyve-dashboard-card__camera-player hv-camera-carousel__stream';
    return stream;
  }

  _attachStream(ent: CarouselEntity, { active = false, buffered = false } = {}): CarouselStream {
    this._ensureStreamMap();
    ...
    if (!stream || !stream.isConnected) {
      if (stream) this._streams.delete(ent.entity_id);
  ...
  _syncSingleStream() {
    ...
    if (!cur || !this._stage) return;
    this._ensureStreamMap();
    for (const entityId of [...this._streams.keys()]) {
      if (this._streams.get(entityId) !== this._stream) this._removeStream(entityId);
    }
  ...
  _reloadEntities() {
    ...
    if (Number.isFinite(idx)) this._index = Math.min(Math.max(0, idx), Math.max(0, this._entities.length - 1));
    else if (this._index >= this._entities.length) this._index = 0;
    this._renderDots();
  }
  ...
  _build() {
    ...
    this._stage = this.querySelector('.hv-camera-carousel__stage') as HTMLElement;
    this._titleEl = this.querySelector('.hv-camera-carousel__title') as HTMLElement;
    this._dotsEl = this.querySelector('.hv-camera-carousel__dots') as HTMLElement;
    this._menu = this.querySelector('.hv-camera-carousel__menu') as HTMLElement;
    this._menuBtn = this.querySelector('.hv-camera-carousel__menu-btn') as HTMLButtonElement;
    this._prevBtn = this.querySelector('.hv-camera-carousel__nav--prev') as HTMLButtonElement;
    this._nextBtn = this.querySelector('.hv-camera-carousel__nav--next') as HTMLButtonElement;
    this._volRow = this.querySelector('.hv-camera-carousel__menu-row--vol') as HTMLElement;
    this._micRow = this.querySelector('.hv-camera-carousel__menu-row--mic') as HTMLElement;
    this._pttBtn = this.querySelector('[data-act="ptt"]') as HTMLButtonElement;
    this._menuHint = this.querySelector('[data-menu-hint]') as HTMLElement;
    this._volSlider = this.querySelector('[data-act="speaker-volume"]') as HTMLInputElement;
    this._spkBtn = this.querySelector('[data-act="speaker-mute"]') as HTMLButtonElement;
    this._micBtn = this.querySelector('[data-act="mic-mute"]') as HTMLButtonElement;
    ...
    this._volSlider.addEventListener('input', (ev) => {
      ev.stopPropagation();
      this._speakerVolume = Number((ev.target as HTMLInputElement).value);
    });
    this._volSlider.addEventListener('change', (ev) => {
      ev.stopPropagation();
      this._postAudio('set_speaker_volume', { volume: Number((ev.target as HTMLInputElement).value) });
    });

    const startPtt = (ev: Event) => { ev.preventDefault(); ev.stopPropagation(); this._startPtt(); };
    const stopPtt = (ev: Event) => { ev.preventDefault(); ev.stopPropagation(); void this._stopPtt(true); };
    ...
    this._viewport = this.querySelector('.hv-camera-carousel__viewport') as HTMLElement;
    this._viewport.addEventListener('touchstart', (ev) => {
      if (this._entities.length < 2) return;
      this._touchStartX = ev.changedTouches?.[0]?.clientX ?? null;
    }, { passive: true });
    this._viewport.addEventListener('touchend', (ev) => {
      if (this._touchStartX == null || this._entities.length < 2) return;
      const endX = ev.changedTouches?.[0]?.clientX;
      if (endX == null) return;
      const delta = endX - this._touchStartX;
      this._touchStartX = null;
      if (Math.abs(delta) < 40) return;
      if (delta < 0) this._setIndex(this._index + 1);
      else this._setIndex(this._index - 1);
    }, { passive: true });
  }
  ...
  _ensureMenuListeners() {
    if (this._menuListenersBound) return;
    this._menuListenersBound = true;
    this._onMenuReposition = () => {
      if (this._menuOpen) this._positionMenu();
    };
    ...
    this._onDocClick = (ev: MouseEvent) => {
      ...
      if (target?.closest('.hv-camera-carousel__menu') || target?.closest('.hv-camera-carousel__menu-btn')) return;
      this._closeMenu();
    };
  ...
  async _loadCapabilities(attempt = 0): Promise<void> {
    const cur = this._current();
    if (!cur) return;
    ...
    try {
      ...
      if (this._menuHint) this._menuHint.textContent = t('dashboard.camera.caps_failed');
    }
  }

  _applyCapabilitiesUi() {
    const caps = this._caps || {};
    if (this._volRow) this._volRow.hidden = !caps.speaker_volume_mutable;
    if (this._micRow) this._micRow.hidden = !caps.microphone_mutable;
    const showPtt = !!caps.supports_talk;
    if (this._pttBtn) this._pttBtn.hidden = !showPtt;
  ...
  _updateMenuUi() {
    if (this._spkBtn) {
      ...
      if (icon) icon.className = this._speakerMuted ? 'fas fa-volume-xmark' : 'fas fa-volume-high';
    ...
    if (this._micBtn) {
      ...
      if (icon) icon.className = this._micMuted ? 'fas fa-microphone-slash' : 'fas fa-microphone';
  ...
  _applySpeakerMute() {
    ...
    if (video) video.muted = this._speakerMuted;
  ...
  _applyMicToCamera() {
    if (!this._caps?.microphone_mutable) return;
    this._postAudio('set_microphone_muted', { enabled: this._micMuted });
  }

  async _postAudio(action: string, payload: Record<string, unknown>): Promise<void> {
    const cur = this._current();
    if (!cur) return;
    try {
      const token = await getCameraStreamToken(cur.entity_id);
  ...
  async _startPtt() {
    if (this._pttActive || !this._caps?.supports_talk) return;
    ...
    try {
      ...
      this._pttRecorder.ondataavailable = (ev) => {
        if (ev.data?.size) this._pttChunks.push(ev.data);
      };
  ...
  async _stopPtt(send = false): Promise<void> {
    ...
    const finalize = async () => {
      ...
      if (!send || !this._pttChunks.length) return;
      const cur = this._current();
      if (!cur) return;
      ...
      try {
        const res = await fetch(
          `/api/cameras/${encodeURIComponent(cur.entity_id)}/talk?token=${encodeURIComponent(token)}`,
          { method: 'POST', headers: { Authorization: `Bearer ${token}` }, body: form },
        );
        if (!res.ok) {
          const err = await res.json().catch(() => ({}));
          if (this._menuHint) this._menuHint.textContent = String((err as { detail?: string }).detail || t('dashboard.camera.talk_error'));
        }
      } catch (_) {
        if (this._menuHint) this._menuHint.textContent = t('dashboard.camera.talk_send_error');
      }
    };
  ...
  _renderDots() {
    ...
    if (this._prevBtn) this._prevBtn.hidden = !multi;
    if (this._nextBtn) this._nextBtn.hidden = !multi;
  ...
  _setIndex(next: number, { force = false, syncAttr = true } = {}): void {
    if (!this._entities.length) return;
    let idx = next;
    if (idx < 0) idx = this._entities.length - 1;
    if (idx >= this._entities.length) idx = 0;
    if (!force && idx === this._index) return;
    this._index = idx;
    if (syncAttr) {
      const idxStr = String(idx);
      if (this.getAttribute('index') !== idxStr) this.setAttribute('index', idxStr);
    }
    this._caps = null;
    this._capsEntity = '';
    this._closeMenu();
  ...
  _entityWebm(ent: CarouselEntity): boolean {
    if (ent.webm != null) return ent.webm;
    return false;
  }
