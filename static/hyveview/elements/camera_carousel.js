...
class HyveCameraCarousel extends HTMLElement {
  ...
  _entityStreamConfig(ent) {
    const live = (window._dashboardCache?.available_entities || []).find((e) => e.entity_id === ent.entity_id);
  ...
  _build() {
    ...
    const stopPtt = (ev) => { ev.preventDefault(); ev.stopPropagation(); this._stopPtt(true); };
  ...
  _ensureMenuListeners() {
    ...
    this._onDocClick = (ev) => {
      ...
      if (ev.target.closest('.hv-camera-carousel__menu') || ev.target.closest('.hv-camera-carousel__menu-btn')) return;
      this._closeMenu();
    };
    ...
    const finalize = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          this._menuHint.textContent = err.detail || t('dashboard.camera.talk_error');
        }
      } catch (_) {
  ...
  _renderDots() {
    ...
    this._dotsEl.innerHTML = multi
      ...
      : '';
    this._dotsEl.querySelectorAll('.hv-camera-carousel__dot').forEach((dot) => {
