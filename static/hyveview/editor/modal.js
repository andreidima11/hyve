...
function _renderPicker(panel) {
  ...
  `;
  panel.querySelector('[data-role=close]').addEventListener('click', () => _close(null));
...
function _renderVisibilityEditor(host, initial) {
  ...
  `;
  const body = host.querySelector('[data-role=vis-body]');
  const condsHost = host.querySelector('[data-role=vis-conds]');
  host.querySelector('[data-role=vis-enabled]').addEventListener('change', (e) => {
    state.enabled = e.target.checked;
    body.style.display = state.enabled ? '' : 'none';
  });
  host.querySelector('[data-role=vis-logic]').addEventListener('change', (e) => { state.logic = e.target.value; });
  ...
  host.querySelector('[data-role=vis-add]').addEventListener('click', () => {
...
function _wireSizePicker(panel) {
  ...
  // Background cells (pure visual; pointer events handled on the container).
  const cells = [];
  ...
  const paint = () => {
    ...
    cells.forEach((cell) => {
      const on = (+cell.dataset.c <= col) && (+cell.dataset.r <= row);
    ...
    presets.forEach((b) => {
      b.dataset.active = (+b.dataset.cols === col && +b.dataset.rows === row) ? 'true' : 'false';
    });
  };
  ...
  const onMove = (e) => { if (dragging) { sizeFromPoint(e.clientX, e.clientY); e.preventDefault(); } };
...
function _renderForm(panel, { mode, card }) {
  ...
  `;
  panel.querySelector('[data-role=close]').addEventListener('click', () => _close(null));
  panel.querySelector('[data-role=cancel]').addEventListener('click', () => _close(null));

  _wireSizePicker(panel);

  const formHost = panel.querySelector('[data-role=form]');
  let form = null;
  if (schema) {
  ...
  const visEditor = _renderVisibilityEditor(panel.querySelector('[data-role=visibility]'), card.visibility || null);
  ...
  panel.querySelector('[data-role=save]').addEventListener('click', () => {
    ...
    const col = Number(panel.querySelector('[data-role=layout-col]').value) || 4;
    const row = Number(panel.querySelector('[data-role=layout-row]').value) || 2;
    const result = {
      id: card.id,
      type: card.type,
