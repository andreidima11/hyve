...
import type {
    HyveviewEditorCard,
    HyveviewEditorOpenOptions,
    HyveviewEditorResult,
...
function _entityFromConfig(config: Record<string, unknown>): string | null {
...
function _close(result: HyveviewEditorResult): void {
  if (_backdrop) { _backdrop.remove(); _backdrop = null; }
  if (_activeResolve) { const r = _activeResolve; _activeResolve = null; r(result); }
}

export function openEditor({ mode = 'add', card = null }: HyveviewEditorOpenOptions = {}): Promise<HyveviewEditorResult> {
  return new Promise((resolve) => {
    _ensureCss();
    if (_activeResolve) _close(null);
    _activeResolve = resolve;

    _backdrop = document.createElement('div');
    _backdrop.className = 'modal-overlay app-modal fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4';
    _backdrop.addEventListener('click', (e) => { if (e.target === _backdrop) _close(null); });
...
function _renderPicker(panel: HTMLElement): void {
  ...
  `;
  _must<HTMLButtonElement>(panel, '[data-role=close]').addEventListener('click', () => _close(null));
  const picker = _must<HTMLDivElement>(panel, '[data-role=picker]');
  for (const meta of HyveviewRegistry.list()) {
    ...
    btn.addEventListener('click', () => {
      ...
      const newCard: HyveviewEditorCard = {
        type: meta.type,
...
function _renderVisibilityEditor(
    host: HTMLElement,
    initial: HyveviewVisibilityConfig | null,
): { read(): HyveviewVisibilityConfig | null } {
  ...
  `;
  const body = _must<HTMLDivElement>(host, '[data-role=vis-body]');
  const condsHost = _must<HTMLDivElement>(host, '[data-role=vis-conds]');
  _must<HTMLInputElement>(host, '[data-role=vis-enabled]').addEventListener('change', (e) => {
    state.enabled = (e.target as HTMLInputElement).checked;
    body.style.display = state.enabled ? '' : 'none';
  });
  _must<HTMLSelectElement>(host, '[data-role=vis-logic]').addEventListener('change', (e) => {
    state.logic = (e.target as HTMLSelectElement).value;
  });
  ...
  const _renderConds = () => {
    ...
    state.conditions.forEach((cond, idx) => {
      ...
      ent.type = 'text'; ent.placeholder = t('hyveview.visibility_entity_id'); ent.value = cond.entity_id || '';
      ...
      for (const o of [['eq','='],['ne','≠'],['gt','>'],['lt','<'],['in','∈']]) {
        ...
        if ((cond.op || 'eq') === o[0]) opt.selected = true;
        op.appendChild(opt);
      }
      op.addEventListener('change', () => { state.conditions[idx].op = op.value; });
      const val = document.createElement('input');
      val.type = 'text'; val.placeholder = t('hyveview.visibility_value'); val.value = cond.value != null ? String(cond.value) : '';
  ...
  _must<HTMLButtonElement>(host, '[data-role=vis-add]').addEventListener('click', () => {
...
function _wireSizePicker(panel: ParentNode): void {
  const preview = panel.querySelector('[data-role=size-preview]') as HTMLElement | null;
  const handle = panel.querySelector('[data-role=size-handle]') as HTMLElement | null;
  const readout = panel.querySelector('[data-role=size-readout]') as HTMLElement | null;
  const colInput = panel.querySelector('[data-role=layout-col]') as HTMLInputElement | null;
  const rowInput = panel.querySelector('[data-role=layout-row]') as HTMLInputElement | null;
  const presets = panel.querySelectorAll('[data-role=size-presets] .hv-size-preset');
  if (!preview || !handle || !colInput || !rowInput) return;
  ...
  const paint = () => {
    ...
    if (readout) readout.textContent = `${col} × ${row}`;
  ...
  const sizeFromPoint = (clientX: number, clientY: number) => {
    const rect = preview.getBoundingClientRect();
    const pad = 4;
  ...
  const onMove = (e: PointerEvent) => { if (dragging) { sizeFromPoint(e.clientX, e.clientY); e.preventDefault(); } };
...
function _renderForm(panel: HTMLElement, { mode, card }: { mode: 'add' | 'edit'; card: HyveviewEditorCard }): void {
  ...
  `;
  _must<HTMLButtonElement>(panel, '[data-role=close]').addEventListener('click', () => _close(null));
  _must<HTMLButtonElement>(panel, '[data-role=cancel]').addEventListener('click', () => _close(null));

  _wireSizePicker(panel);

  const formHost = _must<HTMLDivElement>(panel, '[data-role=form]');
  let form: HyveviewSchemaFormApi | null = null;
  if (schema) {
  ...
  const visEditor = _renderVisibilityEditor(
